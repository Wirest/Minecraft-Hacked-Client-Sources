...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    public void onUpdate() {
        if (this.worldObj.isBlockLoaded(new BlockPos(this.posX, 0.0D, this.posZ))) {
            ...
            if (this.isRiding()) {
                final UpdateEvent updateEvent = new UpdateEvent(this.rotationYaw,
                        this.rotationPitch, this.getEntityBoundingBox().minY, this.onGround, true);
                updateEvent.dispatch();
                if (updateEvent.isCanceled()) {
                    new UpdateEvent(this.rotationYaw,
                            this.rotationPitch, this.getEntityBoundingBox().minY, this.onGround).dispatch();
                    return;
                }
    ...
    public void onUpdateWalkingPlayer() {
        final UpdateEvent event = new UpdateEvent(this.rotationYaw, this.rotationPitch,
                this.getEntityBoundingBox().minY, this.onGround, true);
        event.dispatch();
        if (event.isCanceled()) {
            new UpdateEvent(this.rotationYaw,
                    this.rotationPitch, this.getEntityBoundingBox().minY, this.onGround).dispatch();
        ...
        if (this.isCurrentViewEntity()) {
            this.setRotationYawHead(event.getYaw());
            ...
            if (this.ridingEntity == null) {
                if (flag2 && flag3) {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(this.posX, event.getY(),
                            this.posZ, event.getYaw(), event.getPitch(), event.isOnGround()));
                } else if (flag2) {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(this.posX, event.getY(),
                            this.posZ, event.isOnGround()));
                } else if (flag3) {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer.C05PacketPlayerLook(event.getYaw(),
                            event.getPitch(), event.isOnGround()));
                } else {
                ...
                this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(this.motionX, -999.0D,
                        this.motionZ, event.getYaw(), event.getPitch(), event.isOnGround()));
                flag2 = false;
            }
