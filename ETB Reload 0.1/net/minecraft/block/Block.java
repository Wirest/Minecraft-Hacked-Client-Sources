...
public class Block {
    ...
    public boolean isVisuallyOpaque() {
        if (Client.INSTANCE.getModuleManager().getModuleClass(Xray.class).isEnabled()) {
            if (getIdFromBlock(this) == 16) {
                return true;
            }
            return false;
        }
    ...
    public boolean shouldSideBeRendered(IBlockAccess worldIn, BlockPos pos, EnumFacing side) {
        ...
        if (Client.INSTANCE.getModuleManager().getModuleClass(Xray.class).isEnabled()) {
            Xray xray = (Xray) Client.INSTANCE.getModuleManager().getModuleClass(Xray.class);
            final int myid = getIdFromBlock(this);
            for (Integer block : xray.getBlocks()) {
                if (myid == block.intValue()) {
                    return true;
                } else if (myid == 1 || myid == 3 || myid == 13) {
                    return false;
                }
            }
        }
    ...
    public EnumWorldBlockLayer getBlockLayer() {
        if (Client.INSTANCE.getModuleManager().getModuleClass(Xray.class).isEnabled()) {
            final int myid = getIdFromBlock(this);
            for (int id : ((Xray) Client.INSTANCE.getModuleManager().getModuleClass(Xray.class)).blocks) {
                if (myid == id) {
