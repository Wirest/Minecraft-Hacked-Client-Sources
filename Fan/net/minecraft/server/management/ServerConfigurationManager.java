...
public abstract class ServerConfigurationManager
{
    ...
    public void playerLoggedIn(EntityPlayerMP playerIn)
    {
        ...
        for (EntityPlayerMP entityPlayerMP : this.playerEntityList) {
            playerIn.playerNetServerHandler.sendPacket(new S38PacketPlayerListItem(S38PacketPlayerListItem.Action.ADD_PLAYER, new EntityPlayerMP[]{(EntityPlayerMP) entityPlayerMP}));
        }
    }
    ...
    public EntityPlayerMP createPlayerForUser(GameProfile profile)
    {
        ...
        for (EntityPlayerMP entityPlayerMP : this.playerEntityList) {
            ...
            if (((EntityPlayerMP) entityPlayerMP).getUniqueID().equals(uuid)) {
                list.add((EntityPlayerMP) entityPlayerMP);
            }
        }
    ...
    public void sendPacketToAllPlayersInDimension(Packet packetIn, int dimension)
    {
        for (EntityPlayerMP entityPlayerMP : this.playerEntityList) {
            ...
            if (((EntityPlayerMP) entityPlayerMP).dimension == dimension) {
                ((EntityPlayerMP) entityPlayerMP).playerNetServerHandler.sendPacket(packetIn);
            }
        }
    }
    ...
    public void sendMessageToTeamOrEvryPlayer(EntityPlayer player, IChatComponent message)
    {
        ...
        else
        {
            for (EntityPlayerMP entityPlayerMP : this.playerEntityList) {
                ...
                if (((EntityPlayerMP) entityPlayerMP).getTeam() != team) {
                    ((EntityPlayerMP) entityPlayerMP).addChatMessage(message);
                }
            }
        }
    }
    ...
    public String func_181058_b(boolean p_181058_1_)
    {
        ...
        for (int i = 0; i < ((List)list).size(); ++i)
        {
            ...
            s.append(((EntityPlayerMP) list.get(i)).getName());

            if (p_181058_1_)
            {
                s.append(" (").append(((EntityPlayerMP) list.get(i)).getUniqueID().toString()).append(")");
            }
        }
    ...
    public void sendToAllNearExcept(EntityPlayer p_148543_1_, double x, double y, double z, double radius, int dimension, Packet p_148543_11_)
    {
        for (EntityPlayerMP entityPlayerMP : this.playerEntityList) {
            ...
            if ((EntityPlayerMP) entityPlayerMP != p_148543_1_ && ((EntityPlayerMP) entityPlayerMP).dimension == dimension) {
                double d0 = x - ((EntityPlayerMP) entityPlayerMP).posX;
                double d1 = y - ((EntityPlayerMP) entityPlayerMP).posY;
                double d2 = z - ((EntityPlayerMP) entityPlayerMP).posZ;

                if (d0 * d0 + d1 * d1 + d2 * d2 < radius * radius) {
                    ((EntityPlayerMP) entityPlayerMP).playerNetServerHandler.sendPacket(p_148543_11_);
                }
            }
        }
    }
    ...
    {
        for (EntityPlayerMP entityPlayerMP : this.playerEntityList) {
            this.writePlayerData((EntityPlayerMP) entityPlayerMP);
        }
    }
    ...
    public StatisticsFile getPlayerStatsFile(EntityPlayer playerIn)
    {
        ...
        if (statisticsfile == null)
        {
            ...
            File file2 = new File(file1, Objects.requireNonNull(uuid).toString() + ".json");
