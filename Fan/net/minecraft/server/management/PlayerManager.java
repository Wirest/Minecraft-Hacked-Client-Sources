    ...
    public void updatePlayerInstances()
    {
        ...
        if (i - this.previousTotalWorldTime > 8000L)
        {
            ...
            for (PlayerInstance playerInstance : this.playerInstanceList) {
                ((PlayerInstance) playerInstance).onUpdate();
                ((PlayerInstance) playerInstance).processChunk();
            }
        }
        else
        {
    ...
    class PlayerInstance
    {
        ...
        public void sendToAllPlayersWatchingChunk(Packet thePacket)
        {
            for (EntityPlayerMP entityPlayerMP : this.playersWatchingChunk) {
                ...
                if (!((EntityPlayerMP) entityPlayerMP).loadedChunks.contains(this.chunkCoords)) {
                    ((EntityPlayerMP) entityPlayerMP).playerNetServerHandler.sendPacket(thePacket);
                }
            }
        }
