...
public final class esp2d extends Module {
    ...
    public ModeSetting mode = new ModeSetting("Mode", "Box", "Box", "CS:GO", "Simon", "Joseph", "E-Girl"),
    ...
    public NumberSetting rainbowSpeed = new NumberSetting("Rainbow Speed", 30, 1, 100, 1),
            ...
            opacityChanger = new NumberSetting("Opacity Changer", 10, 1, 40, 0.5),
            pictureSize = new NumberSetting("Picture Size", 10, 1, 40, 1);
    Color color2;

    public esp2d() {
        super("2DESP", Keyboard.KEY_O, Category.RENDER);
        addSettings(outline, mode, healthBar, players, invisibles, entitys, droppedItems, opacityChanger, pictureSize, theme, rainbowSpeed, red, green, blue);
    }
                        ...
                        if (outline) {
                            switch (mode.getSelected()) {
                                ...
                                case "CS:GO":
                                    ...
                                    break;
                                case "Simon":
                                    location = "Fan/simon.jpg";
                                    break;
                                case "Joseph":
                                    location = "Fan/profpic.jpg";
                                    break;
                                case "E-Girl":
                                    location = "Fan/egirl.jpg";
                                    break;
                            }
                            if (mode.is("Joseph") || mode.is("Simon") || mode.is("E-Girl")) {
                                // Take this leakers Unfair#4361
                                float opacity = (float) ((entity.getDistanceToEntity(mc.thePlayer) + opacityChanger.getValue()) / 100);
                                ...
                                float drawX = (float) ((float) Math.abs(posX - endPosX) + pictureSize.getValue()),
                                        drawY = (float) ((float) Math.abs(posX - endPosX) + pictureSize.getValue());

                                Gui.drawModalRectWithCustomSizedTexture((float) posX, (float) (posY), 0, 0, drawX, drawY, drawX, drawY);
