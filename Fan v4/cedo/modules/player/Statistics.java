...
public class Statistics extends Module {
    ...
    private final NumberSetting displayDuration = new NumberSetting("Duration", 12, 3, 30, 1);
    private final NumberSetting scale = new NumberSetting("Scale", 1, 0.8, 2.5, 0.1);
    private final NumberSetting xPos = new NumberSetting("X", 0.1, 0.01, 1, 0.01);
    private final NumberSetting yPos = new NumberSetting("Y", 0.1, 0.01, 1, 0.01);
    ...
    private void showEndScreen(long gameEndTime) {
        ...
        if (statsMap.containsKey("Fly Toggles"))
            statWindow.add("Times Flown", Integer.toString(statsMap.get("Fly Toggles")));
        if (statsMap.containsKey("Speed Duration"))
            statWindow.add("Speed Duration", formatTime(statsMap.get("Speed Duration")));
        if (statsMap.containsKey("Scaffold Blocks Placed"))
            statWindow.add("Scaffold Blocks Placed", Integer.toString(statsMap.get("Scaffold Blocks Placed")));
        if (statsMap.containsKey("Pots Thrown"))
            statWindow.add("Pots Thrown", Integer.toString(statsMap.get("Pots Thrown")));
