...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T>
{
    ...
    public void doRender(T entity, double x, double y, double z, float entityYaw, float partialTicks)
    {
        ...
        try
        {
            ...
            float f7 = /*(entity instanceof EntityPlayerSP) && Fan.getModule(BadScaffold.class).isEnabled() && Fan.getModule(BadScaffold.class).keepRotations.isEnabled()) ? Fan.getModule(BadScaffold.class).lastPitch : */entity.prevRotationPitch + (entity.rotationPitch - entity.prevRotationPitch) * partialTicks;
        ...
        if (flag || flag1) {
            ...
            if (flag2) {
                ...
                if(chams.behindWalls.is("Fade"))
                    chamsColor = RenderUtil.fade(Objects.requireNonNull(color1), 1, 1);
                else if(chams.behindWalls.is("Opposite"))
                    chamsColor = new Color(RenderUtil.getOppositeColor(Objects.requireNonNull(color1).getRGB()));
            ...
            if (flag2) {
                ...
                if (!textured) {
                    final Color chamsColor = color1;

                    glColor4f(Objects.requireNonNull(chamsColor).getRed() / 255.0F, chamsColor.getGreen() / 255.0F, chamsColor.getBlue() / 255.0F, chamsColor.getAlpha() / 255.0F);
                }
