...
public abstract class FileUtils {
    public static List<String> readFile(String file) {
        try {
            ...
            FileUtils.createConfigFile(file);
            e.printStackTrace();
    ...
    public static void writeFile(String file, List<String> newcontent) {
        try {
            ...
            for (String s : newcontent) {
                fw.append(String.valueOf(String.valueOf(s)) + "\r\n");
            }
            fw.close();
        }
        catch (IOException e) {
    ...
    public static void createFile(String name) {
        try {
            File file = new File(Weepcraft.weepcraftDir, String.valueOf(String.valueOf(name)) + ".weep");
            if (!file.exists()) {
            ...
            FileWriter fw = new FileWriter(file);
            fw.append(FileUtils.getDateString());
            fw.close();
        }
        catch (Exception e) {
            e.printStackTrace();
    ...
    public static void createConfigFile(String name) {
        try {
            File file = new File(Weepcraft.configDir, String.valueOf(String.valueOf(name.replace(".weep", ""))) + ".weep");
            if (!file.exists()) {
                PrintWriter printWriter = new PrintWriter(new FileWriter(file));
                printWriter.println();
                printWriter.close();
    ...
    public static void createAccFile(String name) {
        try {
            File file = new File(Weepcraft.accDir, String.valueOf(String.valueOf(name)) + ".weep");
            if (!file.exists()) {
                PrintWriter printWriter = new PrintWriter(new FileWriter(file));
                printWriter.println();
                printWriter.close();
