...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    public void onUpdate()
    {
        if (this.world.isBlockLoaded(new BlockPos(this.posX, 0.0D, this.posZ)))
        {
            ...
            if (this.isRiding())
            {
            	  EventPreMotionUpdates event = new EventPreMotionUpdates(this.rotationYaw, this.rotationPitch);
            ...
            EventPostMotionUpdates event1 = new EventPostMotionUpdates();
    ...
    public void sendChatMessage(String message)
    {
        ...
        if (!message.startsWith(".")) {
            if (message.startsWith("@")) {
                final IrcManager var11 = Weepcraft.ircManager;
                if (var11.isConnected()) {
                    var11.sendMessage(MinecraftServer.getIRCChannel(), message.replace("@", ""));
                    var11.getLines().add(new IrcLine(var11.getChatLine(true), message.replace("@", ""), var11.getName(), false));
                    var11.setUnreadMessages(true);
                }
                else {
                    Wrapper.tellPlayerIRC("Not Connected To IRC");
                }
            }
            else {
            ...
            for (final Command irc : CommandManager.cmds) {
                final String[] var13 = irc.names;
                for (int var14 = irc.names.length, var15 = 0; var15 < var14; ++var15) {
                    final String n = var13[var15];
                    final String mname = message.split(" ")[0].replace(".", "");
                    if (mname.equalsIgnoreCase(n)) {
                        try {
                            if (!irc.runCommand(message, message.replace("." + mname, "").split(" "))) {
                                Wrapper.tellPlayer("Invalid args. " + irc.syntax);
                            }
                        }
    ...
    {
    	 EventPrePlayerUpdate event = new EventPrePlayerUpdate(0.0F, 0.0F);
    	    EventManager.call(event);
        ...
        if (this.isRidingHorse())
        {
            ...
            if (this.horseJumpPowerCounter < 0)
            {
                ...
                EventPostPlayerUpdate event1 = new EventPostPlayerUpdate();
                EventManager.call(event1);
            }
    ...
    {
    	 EventMove event = new EventMove(p_70091_2_, p_70091_4_, p_70091_6_);
    	    EventManager.call(event);
    	    double d0 = this.posX;
    	    double d1 = this.posZ;
    	    p_70091_2_ = event.x;
