...
import me.rigamortis.faurax.events.EventLiquidBoundingBox;
...
public abstract class BlockLiquid extends Block
{
    ...
    public AxisAlignedBB getCollisionBoundingBox(World worldIn, BlockPos pos, IBlockState state) {
        EventLiquidBoundingBox event = new EventLiquidBoundingBox(pos.getY());
        EventManager.call(event);
        if (event.isCancelled()) {
            return new AxisAlignedBB((double)pos.getX() + this.minX, (double)pos.getY() - 1.0, (double)pos.getZ() + this.minZ, (double)pos.getX() + this.maxX, event.getHeight(), (double)pos.getZ() + this.maxZ);
        }
        return null;
    }
