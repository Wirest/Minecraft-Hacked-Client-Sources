...
import me.rigamortis.faurax.events.EventClickBlock;
import me.rigamortis.faurax.events.EventDamageBlock;
import me.rigamortis.faurax.events.EventPlaceBlock;
import me.rigamortis.faurax.events.EventPlayerReach;
...
public class PlayerControllerMP
{
    ...
    {
    	 EventDamageBlock event = new EventDamageBlock(p_180512_1_, p_180512_2_);
         EventManager.call(event);
         if (event.isCancelled()) {
             return false;
         }
    ...
    public float getBlockReachDistance() {
        EventPlayerReach event = new EventPlayerReach(this.currentGameType.isCreative() ? 5.0f : 4.5f);
        EventManager.call(event);
    ...
    {
    	EventPlaceBlock event = new EventPlaceBlock(p_178890_4_, p_178890_5_);
        EventManager.call(event);
        if (event.isCancelled()) {
            return false;
        }
