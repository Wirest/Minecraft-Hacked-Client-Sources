...
public class Dark extends Theme implements RenderHelper
{
    ...
    public Dark() {
        ...
        this.font = new CustomFont("Impact", 18.0f);
    }
    ...
    public void panelDraw(final Panel panel, final int mouseX, final int mouseY) {
        if (panel.isVisible()) {
            final GuiUtils guiUtils = Dark.guiUtils;
            GuiUtils.drawGradientRect(panel.x + panel.dragX, panel.y + panel.dragY, panel.dragX + panel.x + 110.0f + 11 * panel.scrollBars.size(), panel.dragY + panel.y + 14.0f + 13 * panel.buttons.size() + 13 * panel.sliders.size() + 13 * panel.checkBoxes.size() + 13 * panel.dropDowns.size() + 170 * panel.scrollBars.size() + 100 * panel.radar.size(), -1725618907, -1725421528);
            final GuiUtils guiUtils2 = Dark.guiUtils;
            GuiUtils.drawGradientRect(panel.x + panel.dragX, panel.y + panel.dragY, panel.dragX + panel.x + 110.0f + 11 * panel.scrollBars.size(), panel.dragY + panel.y + 13.0f, -13619152, -14342875);
            Dark.guiUtils.drawOutlineRect(panel.x + panel.dragX, panel.y + panel.dragY, panel.dragX + panel.x + 110.0f + 11 * panel.scrollBars.size(), panel.dragY + panel.y + 14.0f + 13 * panel.buttons.size() + 13 * panel.sliders.size() + 13 * panel.checkBoxes.size() + 13 * panel.dropDowns.size() + 170 * panel.scrollBars.size() + 100 * panel.radar.size(), 1345335344);
            this.font.drawString(panel.name, panel.x + panel.dragX + 3.0f, panel.y + panel.dragY - 1.5f, CustomFont.FontType.SHADOW_THIN, -1);
            GL11.glEnable(3089);
    ...
    public void buttonDraw(final Button button, final Panel panel, final float x, final float y) {
        final GuiUtils guiUtils = Dark.guiUtils;
        GuiUtils.drawGradientRect(button.x + panel.dragX + panel.x + 1.0f, button.y + panel.dragY + panel.y, button.x + panel.dragX + panel.x + 109.0f, button.y + panel.dragY + panel.y + 12.0f, -2009778891, -2010107856);
        Dark.guiUtils.drawOutlineRect(button.x + panel.dragX + panel.x + 1.0f, button.y + panel.dragY + panel.y, button.x + panel.dragX + panel.x + 109.0f, button.y + panel.dragY + panel.y + 12.0f, -2010107856);
        this.font.drawString(button.name, button.x + panel.dragX + panel.x + 3.0f, button.y + panel.dragY + panel.y - 2.0f, CustomFont.FontType.SHADOW_THIN, -7829368);
        if (x >= panel.x + panel.dragX + button.x && y >= panel.dragY + panel.y + button.y && x <= panel.dragX + panel.x + button.x + 110.0f && y <= panel.dragY + panel.y + button.y + 12.0f) {
            this.font.drawString(button.name, button.x + panel.dragX + panel.x + 3.0f, button.y + panel.dragY + panel.y - 2.0f, CustomFont.FontType.SHADOW_THIN, -1);
        }
        if (button.enabled) {
            this.font.drawString(button.name, button.x + panel.dragX + panel.x + 3.0f, button.y + panel.dragY + panel.y - 2.0f, CustomFont.FontType.SHADOW_THIN, -15649472);
        }
        if (x >= panel.x + panel.dragX + button.x && y >= panel.dragY + panel.y + button.y && x <= panel.dragX + panel.x + button.x + 110.0f && y <= panel.dragY + panel.y + button.y + 12.0f && button.enabled) {
            this.font.drawString(button.name, button.x + panel.dragX + panel.x + 3.0f, button.y + panel.dragY + panel.y - 2.0f, CustomFont.FontType.SHADOW_THIN, -15646651);
        }
        for (final Module mod : Client.getModules().mods) {
            if (mod.getName().equalsIgnoreCase(button.name)) {
                if (mod.isToggled()) {
    ...
    public void checkBoxDraw(final CheckBox checkBox, final Panel panel, final float x, final float y) {
        ...
        this.font.drawString(checkBox.name, checkBox.x + panel.dragX + panel.x + 2.0f, checkBox.y + panel.dragY + panel.y - 2.0f, CustomFont.FontType.SHADOW_THIN, -6710887);
        if (x >= panel.x + panel.dragX + checkBox.x && y >= panel.dragY + panel.y + checkBox.y && x <= panel.dragX + panel.x + checkBox.x + 110.0f && y <= panel.dragY + panel.y + checkBox.y + 12.0f) {
            ...
            this.font.drawString(checkBox.name, checkBox.x + panel.dragX + panel.x + 2.0f, checkBox.y + panel.dragY + panel.y - 2.0f, CustomFont.FontType.SHADOW_THIN, -1);
        }
        if (checkBox.enabled) {
            final GuiUtils guiUtils3 = Dark.guiUtils;
            ...
            this.font.drawString(checkBox.name, checkBox.x + panel.dragX + panel.x + 2.0f, checkBox.y + panel.dragY + panel.y - 2.0f, CustomFont.FontType.SHADOW_THIN, -1);
            final GuiUtils guiUtils4 = Dark.guiUtils;
        ...
        if (x >= panel.x + panel.dragX + checkBox.x && y >= panel.dragY + panel.y + checkBox.y && x <= panel.dragX + panel.x + checkBox.x + 110.0f && y <= panel.dragY + panel.y + checkBox.y + 12.0f && checkBox.enabled) {
            final GuiUtils guiUtils5 = Dark.guiUtils;
            ...
            this.font.drawString(checkBox.name, checkBox.x + panel.dragX + panel.x + 2.0f, checkBox.y + panel.dragY + panel.y - 2.0f, CustomFont.FontType.SHADOW_THIN, -1);
            final GuiUtils guiUtils6 = Dark.guiUtils;
            GuiUtils.drawCheck((int)checkBox.x + (int)panel.dragX + (int)panel.x + 99, (int)checkBox.y + (int)panel.dragY + (int)panel.y + 1, -1);
        }
    ...
    public void dropDownDraw(final DropDown dropDown, final Panel panel, final float x, final float y) {
        ...
        this.font.drawString(dropDown.name, dropDown.x + panel.dragX + panel.x + 4.0f, dropDown.y + panel.dragY + panel.y - 2.0f, CustomFont.FontType.SHADOW_THIN, -6710887);
        final GuiUtils guiUtils3 = Dark.guiUtils;
        ...
        if (x >= panel.x + panel.dragX + dropDown.x && y >= panel.dragY + panel.y + dropDown.y && x <= panel.dragX + panel.x + dropDown.x + 110.0f && y <= panel.dragY + panel.y + dropDown.y + 12.0f) {
            ...
            final GuiUtils guiUtils6 = Dark.guiUtils;
            GuiUtils.drawTriangle(dropDown.x + panel.dragX + panel.x + 103.5f, dropDown.y + panel.dragY + panel.y + 7.0f, 3.5f, state, 1.0f, -871296751);
            final GuiUtils guiUtils7 = Dark.guiUtils;
            GuiUtils.drawTriangle(dropDown.x + panel.dragX + panel.x + 103.0f, dropDown.y + panel.dragY + panel.y + 6.5f, 3.5f, state, 1.0f, -1);
            this.font.drawString(dropDown.name, dropDown.x + panel.dragX + panel.x + 4.0f, dropDown.y + panel.dragY + panel.y - 2.0f, CustomFont.FontType.SHADOW_THIN, -1);
        }
        if (dropDown.open) {
            for (final DropDownButton b : dropDown.buttons) {
                b.draw(x, y);
    ...
    public void dropDownButtonDraw(final DropDownButton dropDownButton, final DropDown dropDown, final Panel panel, final float x, final float y) {
        ...
        this.font.drawString(dropDownButton.name, dropDownButton.x + panel.dragX + 4.0f, dropDownButton.y + panel.dragY - 2.0f, CustomFont.FontType.SHADOW_THIN, -6710887);
        if (x >= panel.x + panel.dragX + dropDownButton.x && y >= panel.dragY + panel.y + dropDownButton.y && x <= panel.dragX + panel.x + dropDownButton.x + 110.0f && y <= panel.dragY + panel.y + dropDownButton.y + 12.0f) {
            ...
            this.font.drawString(dropDownButton.name, dropDownButton.x + panel.dragX + 4.0f, dropDownButton.y + panel.dragY - 2.0f, CustomFont.FontType.SHADOW_THIN, -1);
        }
    ...
    public void ScrollBarDraw(final ScrollBar scrollBar, final Panel panel, final float x, final float y) {
        ...
        Dark.guiUtils.drawOutlineRect(scrollBar.x + panel.dragX + panel.x + 110.0f, scrollBar.y + panel.dragY + panel.y + scrollBar.dragY + 1.0f, scrollBar.x + panel.dragX + panel.x + 119.0f, scrollBar.y + panel.dragY + panel.y + 39.0f + scrollBar.dragY, 1999646768);
    ...
    public void ScrollBarButtonDraw(final ScrollBarButton button, final ScrollBar scrollBar, final Panel panel, final float x, final float y) {
        final GuiUtils guiUtils = Dark.guiUtils;
        GuiUtils.drawGradientRect(button.x + panel.dragX + 2.0f + scrollBar.x, button.y + panel.dragY + scrollBar.y - scrollBar.dragY, button.x + panel.dragX + 108.0f + scrollBar.x, button.y + panel.dragY + 12.0f + scrollBar.y - scrollBar.dragY, -2009778891, -2010107856);
        Dark.guiUtils.drawOutlineRect(button.x + panel.dragX + 2.0f + scrollBar.x, button.y + panel.dragY + scrollBar.y - scrollBar.dragY, button.x + panel.dragX + 108.0f + scrollBar.x, button.y + panel.dragY + 12.0f + scrollBar.y - scrollBar.dragY, -2144325584);
        this.font.drawString(button.name, button.x + panel.dragX + 3.0f + scrollBar.x, button.y + panel.dragY - 2.0f + scrollBar.y - scrollBar.dragY, CustomFont.FontType.SHADOW_THIN, -6710887);
        if (button.enabled) {
            this.font.drawString(button.name, button.x + panel.dragX + 3.0f + scrollBar.x, button.y + panel.dragY - 2.0f + scrollBar.y - scrollBar.dragY, CustomFont.FontType.SHADOW_THIN, -15649472);
        }
        if (x >= panel.x + panel.dragX + scrollBar.x + 1.0f && y >= panel.dragY + panel.y + scrollBar.y + 1.0f && x <= panel.dragX + panel.x + scrollBar.x + 109.0f && y <= panel.dragY + panel.y + scrollBar.y + 170.0f) {
            if (x >= scrollBar.x + panel.dragX + button.x && y >= panel.dragY + scrollBar.y + button.y - scrollBar.dragY && x <= panel.dragX + scrollBar.x + button.x + 110.0f && y <= panel.dragY + scrollBar.y + button.y + 12.0f - scrollBar.dragY) {
                this.font.drawString(button.name, button.x + panel.dragX + 3.0f + scrollBar.x, button.y + panel.dragY - 2.0f + scrollBar.y - scrollBar.dragY, CustomFont.FontType.SHADOW_THIN, -1);
            }
            if (x >= scrollBar.x + panel.dragX + button.x && y >= panel.dragY + scrollBar.y + button.y - scrollBar.dragY && x <= panel.dragX + scrollBar.x + button.x + 110.0f && y <= panel.dragY + scrollBar.y + button.y + 12.0f - scrollBar.dragY && button.enabled) {
                this.font.drawString(button.name, button.x + panel.dragX + 3.0f + scrollBar.x, button.y + panel.dragY - 2.0f + scrollBar.y - scrollBar.dragY, CustomFont.FontType.SHADOW_THIN, -15646651);
            }
        }
        for (final Panel p : Client.getGUI().panels) {
            if (p.name.equalsIgnoreCase(button.name)) {
    ...
    public void SliderDraw(final Slider slider, final Panel panel, final float x, final float y) {
        ...
        Dark.guiUtils.drawOutlineRect(slider.x + panel.dragX + panel.x + 1.0f, slider.y + panel.dragY + panel.y, slider.x + panel.dragX + panel.x + 109.0f, slider.y + panel.dragY + panel.y + 12.0f, 1714434096);
        final GuiUtils guiUtils2 = Dark.guiUtils;
        ...
        this.font.drawString(slider.name, slider.x + panel.dragX + panel.x + 3.0f, slider.y + panel.dragY + panel.y - 2.0f, CustomFont.FontType.SHADOW_THIN, -1);
        this.font.drawString(new StringBuilder(String.valueOf(percent)).toString(), slider.x + panel.dragX + panel.x + 110.0f - this.font.getStringWidth(new StringBuilder(String.valueOf(percent)).toString()) - 2.0f, slider.y + panel.dragY + panel.y - 2.0f, CustomFont.FontType.SHADOW_THIN, -1);
