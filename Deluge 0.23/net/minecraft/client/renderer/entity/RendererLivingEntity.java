...
import com.ihl.client.module.ESP;
import com.ihl.client.module.Friends;
...
public abstract class RendererLivingEntity extends Render
{
    ...
    {
    	EventEntityRender event = new EventEntityRender(Event.Type.PRE, this, p_76986_1_, p_76986_2_, p_76986_4_, p_76986_6_);
        ...
        try
        {
            ...
            else
            {
                ...
                ESP esp = (ESP) Module.get("esp");
                String mode = Option.get(esp.options, "mode").CHOICE();
                float width = (float)Option.get(esp.options, "width").DOUBLE();
                ...
                if (esp.active) {
                    ...
                    String type = EntityUtil.getEntityTypeName(p_76986_1_);
                    if (type != null) {
                        hex = Option.get(esp.options, type, "color").STRING();
                        enabled = Option.get(esp.options, type).BOOLEAN();
                    }

                    int hexColor = ColorUtil.intFromHex(hex);
                    Color color = rainbow ? ColorUtil.rainbow((long) (p_76986_1_.posX * p_76986_1_.posZ * 500000L), 1f) : new Color(hexColor);

                    Friends friends = (Friends) Module.get("friends");
                    if (friends.active && Friends.isFriend(p_76986_1_.getName())) {
                    ...
                    float[] rgba = ColorUtil.getRGBA(color.getRGB());

                    OpenGlHelper.setLightmapTextureCoords(OpenGlHelper.lightmapTexUnit, (float)a2 / 1.0F, (float)a3 / 1.0F);
                    GL11.glColor4f(rgba[0], rgba[1], rgba[2], rgba[3]);
                    GL11.glLineWidth(width);

                    ModelRenderer[] parts = null;
                    boolean[] clothing = new boolean[6];

                    if (this.mainModel instanceof ModelPlayer) {
                        parts = new ModelRenderer[clothing.length];

                        parts[0] = ((ModelPlayer) this.mainModel).bipedHeadwear;
                        parts[1] = ((ModelPlayer) this.mainModel).bipedBodywear;
                        parts[2] = ((ModelPlayer) this.mainModel).bipedLeftArmwear;
                        parts[3] = ((ModelPlayer) this.mainModel).bipedLeftLegwear;
                        parts[4] = ((ModelPlayer) this.mainModel).bipedRightArmwear;
                        parts[5] = ((ModelPlayer) this.mainModel).bipedRightLegwear;

                        for(int i = 0; i < parts.length; i++) {
                            clothing[i] = parts[i].showModel;
                            parts[i].showModel = false;
                        }
                    }
                    ...
                    if (enabled) {
                        switch (mode.toLowerCase()) {
                            ...
                            case "outline":
                                ...
                                RenderUtil.stencilClear();
                                this.renderModel(p_76986_1_, var17, var16, var14, var12, var20, 0.0625F);

                                RenderUtil.stencilWrite();
                                GL11.glLineWidth(2F);
                    ...
                    if (parts != null) {
                        for(int i = 0; i < parts.length; i++) {
                            parts[i].showModel = clothing[i];
                        }
                    }

                    GL11.glColor4f(1, 1, 1, 1);

                    a1 = p_76986_1_.getBrightnessForRender(p_76986_9_);

                    if (p_76986_1_.isBurning()) {
                        a1 = 15728880;
                    }
