...
public abstract class Entity implements ICommandSender
{
    ...
    public void moveEntity(double x, double y, double z)
    {
        ...
        else
        {
            ...
            EventSneaking sneak = new EventSneaking();
        	sneak.setType(EventType.PRE);
        	sneak.setX(x);
        	sneak.setY(y);
        	sneak.setZ(z);
        	sneak.worldObj = this.worldObj;
        	sneak.boundingBox = this.getEntityBoundingBox();
        	sneak.entity = this;
        	sneak.d3 = d3;
        	sneak.d5 = d5;
        	sneak.sneaking = flag;
        	SpicyClient.onEvent(sneak);
        	flag = sneak.sneaking;
            ...
            if (flag)
            {
            	...
            	if (sneak.isCanceled()) {
	                ...
	                for (d6 = 0.05D; x != 0.0D && this.worldObj.getCollidingBoundingBoxes(this, this.getEntityBoundingBox().offset(x, sneak.offset, 0.0D)).isEmpty(); d3 = x)
	                {
	                    if (x < d6 && x >= -d6)
	                    {
	                        x = 0.0D;
	                        sneak.postEdgeOfBlock = true;
	                    }
	                    else if (x > 0.0D)
	                    {
	                        x -= d6;
	                ...
	                for (; z != 0.0D && this.worldObj.getCollidingBoundingBoxes(this, this.getEntityBoundingBox().offset(0.0D, sneak.offset, z)).isEmpty(); d5 = z)
	                {
	                    if (z < d6 && z >= -d6)
	                    {
	                        z = 0.0D;
	                ...
	                for (; x != 0.0D && z != 0.0D && this.worldObj.getCollidingBoundingBoxes(this, this.getEntityBoundingBox().offset(x, sneak.offset, z)).isEmpty(); d5 = z)
	                {
	                    if (x < d6 && x >= -d6)
	                    {
	                        x = 0.0D;
            ...
            sneak.setType(EventType.POST);
            SpicyClient.onEvent(sneak);
            if (sneak.revertFlagAfter) {
