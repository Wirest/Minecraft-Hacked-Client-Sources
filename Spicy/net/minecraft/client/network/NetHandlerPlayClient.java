...
public class NetHandlerPlayClient implements INetHandlerPlayClient
{
    ...
    public void handlePlayerPosLook(S08PacketPlayerPosLook packetIn)
    {
        ...
        EventServerSetYawAndPitch event = new EventServerSetYawAndPitch();
        event.setType(EventType.PRE);
        SpicyClient.onEvent(event);
        
        if (event.isCanceled()) {
        	entityplayer.setPositionAndRotation(d0, d1, d2, Minecraft.getMinecraft().thePlayer.rotationYaw, Minecraft.getMinecraft().thePlayer.rotationPitch);
        	if (event.notify) {
        		Command.sendPrivateChatMessage("The server tried to set your yaw/pitch but it was canceled");
        	}
        }else {
    ...
    public void handleTimeUpdate(S03PacketTimeUpdate packetIn)
    {
        ...
        EventServerSettingWorldTime event = new EventServerSettingWorldTime();
        event.setType(EventType.PRE);
        SpicyClient.onEvent(event);
