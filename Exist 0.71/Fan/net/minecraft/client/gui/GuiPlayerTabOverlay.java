...
public class GuiPlayerTabOverlay extends Gui
{
    ...
    public static List<EntityPlayer> getPlayers(){
        List<NetworkPlayerInfo> list = field_175252_a.<NetworkPlayerInfo>sortedCopy(Fan.mc.thePlayer.sendQueue.getPlayerInfoMap());
        List<EntityPlayer> players = new ArrayList<>();
        for(NetworkPlayerInfo player : list) {
        	if(player != null) {
        	players.add(Fan.mc.theWorld.getPlayerEntityByName(player.getGameProfile().getName()));
        	}
        }
        return players;
    }
    ...
    public void renderPlayerlist(int width, Scoreboard scoreboardIn, ScoreObjective scoreObjectiveIn)
    {
        ...
        if (list1 != null)
        {
            drawRect(width / 2 - l1 / 2 - 1, k1 - 1, width / 2 + l1 / 2 + 1, k1 + list1.size() * FontUtil.cleanmedium.getHeight(), Integer.MIN_VALUE);

            for (String s3 : list1)
            {
                int i2 = this.mc.fontRendererObj.getStringWidth(s3);
                FontUtil.cleanmedium.drawStringWithShadow(s3, (float)(width / 2 - i2 / 2), (float)k1, -1);
                k1 += FontUtil.cleanmedium.getHeight();
            }
        ...
        for (int k4 = 0; k4 < l3; ++k4)
        {
            ...
            if (k4 < list.size())
            {
                ...
                if (networkplayerinfo1.getGameType() == WorldSettings.GameType.SPECTATOR)
                {
                    ...
                    FontUtil.cleanmedium.drawStringWithShadow(s1, (float)j2, (float)k2, -1862270977);
                }
                else
                {
                    FontUtil.cleanmedium.drawStringWithShadow(s1, (float)j2, (float)k2, -1);
                }
        ...
        {
            k1 = k1 + i4 * 9 + 1;
            drawRect(width / 2 - l1 / 2 - 1, k1 - 1, width / 2 + l1 / 2 + 1, k1 + list2.size() * FontUtil.cleanmedium.getHeight(), Integer.MIN_VALUE);

            for (String s4 : list2)
            {
                int j5 = this.mc.fontRendererObj.getStringWidth(s4);
                FontUtil.cleanmedium.drawStringWithShadow(s4, (float)(width / 2 - j5 / 2), (float)k1, -1);
                k1 += FontUtil.cleanmedium.getHeight();
            }
        }
    }
    ...
    private void drawScoreboardValues(ScoreObjective p_175247_1_, int p_175247_2_, String p_175247_3_, int p_175247_4_, int p_175247_5_, NetworkPlayerInfo p_175247_6_)
    {
        ...
        if (p_175247_1_.getRenderType() == IScoreObjectiveCriteria.EnumRenderType.HEARTS)
        {
            ...
            if (j > 0)
            {
                ...
                else
                {
                    ...
                    FontUtil.cleanmedium.drawStringWithShadow(s, (float)((p_175247_5_ + p_175247_4_) / 2 - this.mc.fontRendererObj.getStringWidth(s) / 2), (float)p_175247_2_, i1);
                }
            }
        }
        else
        {
            ...
            FontUtil.cleanmedium.drawStringWithShadow(s1, (float)(p_175247_5_ - this.mc.fontRendererObj.getStringWidth(s1)), (float)p_175247_2_, 16777215);
        }
    }
