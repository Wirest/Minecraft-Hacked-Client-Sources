...
public abstract class World implements IBlockAccess
{
    ...
    public void updateEntities()
    {
        ...
        for (Entity value : this.unloadedEntityList) {
            int j = ((Entity) value).chunkCoordX;
            int l1 = ((Entity) value).chunkCoordZ;

            if (((Entity) value).addedToChunk && this.isChunkLoaded(j, l1, true)) {
                this.getChunkFromChunkCoords(j, l1).removeEntity((Entity) value);
            }
        }
        ...
        if (!this.addedTileEntityList.isEmpty())
        {
            for (TileEntity tileEntity : this.addedTileEntityList) {
                ...
                if (!((TileEntity) tileEntity).isInvalid()) {
                    if (!this.loadedTileEntityList.contains((TileEntity) tileEntity)) {
                        this.addTileEntity((TileEntity) tileEntity);
                    }

                    if (this.isBlockLoaded(((TileEntity) tileEntity).getPos())) {
                        this.getChunkFromBlockCoords(((TileEntity) tileEntity).getPos()).addTileEntity(((TileEntity) tileEntity).getPos(), (TileEntity) tileEntity);
                    }

                    this.markBlockForUpdate(((TileEntity) tileEntity).getPos());
                }
            }
    ...
    public boolean checkNoEntityCollision(AxisAlignedBB bb, Entity entityIn)
    {
        ...
        for (Entity value : list) {
            ...
            if (!((Entity) value).isDead && ((Entity) value).preventEntitySpawning && (Entity) value != entityIn && (entityIn == null || entityIn.ridingEntity != (Entity) value && entityIn.riddenByEntity != (Entity) value)) {
                return false;
            }
        }
        ...
        else
        {
            ...
            if (this.processingLoadedTiles)
            {
                for (TileEntity tileEntity : this.addedTileEntityList) {
                    ...
                    if (!((TileEntity) tileEntity).isInvalid() && ((TileEntity) tileEntity).getPos().equals(pos)) {
    ...
    public static boolean doesBlockHaveSolidTopSurface(IBlockAccess blockAccess, BlockPos pos)
    {
        ...
        Block block = iblockstate.getBlock();
        return block.getMaterial().isOpaque() && block.isFullCube() || (block instanceof BlockStairs ? iblockstate.getValue(BlockStairs.HALF) == BlockStairs.EnumHalf.TOP : (block instanceof BlockSlab ? iblockstate.getValue(BlockSlab.HALF) == BlockSlab.EnumBlockHalf.TOP : (block instanceof BlockHopper || (block instanceof BlockSnow && (Integer) iblockstate.getValue(BlockSnow.LAYERS) == 7))));
    }
    ...
    protected void setActivePlayerChunksAndCheckLight()
    {
        ...
        for (EntityPlayer playerEntity : this.playerEntities) {
            int j = MathHelper.floor_double(((EntityPlayer) playerEntity).posX / 16.0D);
            int k = MathHelper.floor_double(((EntityPlayer) playerEntity).posZ / 16.0D);
    ...
    public <T extends Entity> List<T> getEntities(Class <? extends T > entityType, Predicate <? super T > filter)
    {
        ...
        for (Entity entity : (List<Entity>) this.loadedEntityList)
        {
    ...
    public EntityPlayer getClosestPlayer(double x, double y, double z, double distance)
    {
        ...
        for (EntityPlayer playerEntity : this.playerEntities) {
            ...
            if (EntitySelectors.NOT_SPECTATING.apply((EntityPlayer) playerEntity)) {
                double d1 = ((EntityPlayer) playerEntity).getDistanceSq(x, y, z);
    ...
    public boolean isAnyPlayerWithinRangeAt(double x, double y, double z, double range)
    {
        for (EntityPlayer playerEntity : this.playerEntities) {
            ...
            if (EntitySelectors.NOT_SPECTATING.apply((EntityPlayer) playerEntity)) {
                double d0 = ((EntityPlayer) playerEntity).getDistanceSq(x, y, z);
    ...
    public EntityPlayer getPlayerEntityByName(String name)
    {
        for (EntityPlayer playerEntity : this.playerEntities) {
            ...
            if (name.equals(((EntityPlayer) playerEntity).getName())) {
    ...
    public EntityPlayer getPlayerEntityByUUID(UUID uuid)
    {
        for (EntityPlayer playerEntity : this.playerEntities) {
            ...
            if (uuid.equals(((EntityPlayer) playerEntity).getUniqueID())) {
                return (EntityPlayer) playerEntity;
            }
        }
    ...
    {
        for (IWorldAccess worldAccess : this.worldAccesses) {
            ((IWorldAccess) worldAccess).broadcastSound(p_175669_1_, pos, p_175669_3_);
        }
    }
    ...
    public void playAuxSFXAtEntity(EntityPlayer player, int sfxType, BlockPos pos, int p_180498_4_)
    {
        ...
        {
            for (IWorldAccess worldAccess : this.worldAccesses) {
                ((IWorldAccess) worldAccess).playAuxSFX(player, sfxType, pos, p_180498_4_);
            }
        }
        catch (Throwable throwable)
        {
    ...
    {
        for (IWorldAccess worldAccess : this.worldAccesses) {
            ((IWorldAccess) worldAccess).sendBlockBreakProgress(breakerId, pos, progress);
        }
    }
