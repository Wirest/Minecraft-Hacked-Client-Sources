...
import dev.astroclient.client.feature.impl.combat.KillAura;
import dev.astroclient.client.feature.impl.visuals.Chams;
...
public class ItemRenderer {
    ...
    public void renderItemInFirstPerson(float partialTicks) {
        Chams chams = Client.INSTANCE.featureManager.chams;
        ...
        if (this.itemToRender != null) {
            KillAura killAura = (KillAura) Client.INSTANCE.featureManager.get(KillAura.class);
            if (this.itemToRender.getItem() instanceof ItemMap) {
                ...
                switch (ItemRenderer.ItemRenderer$1.field_178094_a[enumaction.ordinal()]) {
                    ...
                    case 4:
                        ...
                        if (Client.INSTANCE.featureManager.animation.getState()) {
                            float equipProgress = Client.INSTANCE.featureManager.animation.equipProgress.getValue() ? f : 0;
                            GL11.glTranslated(Client.INSTANCE.featureManager.animation.x.getValue(), Client.INSTANCE.featureManager.animation.y.getValue(), 1.0 * Client.INSTANCE.featureManager.animation.z.getValue());
                            switch (Client.INSTANCE.featureManager.animation.mode.getValue()) {
                                case "1.7":
                                    this.transformFirstPersonItem(equipProgress / 3, f1);
                                    this.func_178103_d();
                                    break;
                                case "Astro":
                                    this.transformFirstPersonItem(equipProgress / 2, f1);
                                ...
                                case "Slide":
                                    ...
                                    this.func_178103_d();
                                    final float var10 = MathHelper.sin(MathHelper.sqrt_float(f1) * MathHelper.PI);
                                    GlStateManager.translate(0, 0.4f, 0);
                                    GlStateManager.rotate(-var10 * (float) 70.0 / 2.0f, -8.0f, 0.4f, 9.0f);
                                    GlStateManager.rotate(-var10 * (float) 45, 1.5f, -0.4f, -0.0f);
                                    break;
                                default:
                                    this.transformFirstPersonItem(equipProgress, 0);
                                    this.func_178103_d();
            ...
            if (chams.isEnabled() && chams.hands.getValue()) {
                GL11.glDisable(GL_ALPHA_TEST);
                GL11.glDisable(GL_TEXTURE_2D);
                GL11.glDisable(GL_LIGHTING);
                GL11.glEnable(GL_BLEND);
                Color color = Color.getHSBColor(chams.hiddenHue.getColor() / 255.0F, .8F, 1.0F);
                if (chams.colorMode.getValue().equalsIgnoreCase("Teams"))
                    color = new Color(14, 61, 75);


                OpenGlHelper.setLightmapTextureCoords(OpenGlHelper.lightmapTexUnit, 240.0f, 240.0f);
                if (chams.colorMode.getValue().equalsIgnoreCase("Custom"))
                    GL11.glColor4f(chams.handColor.getColor().getRed() / 255.0F, chams.handColor.getColor().getGreen() / 255.0F, chams.handColor.getColor().getBlue() / 255.0F, chams.handOpacity.getValue());
                else
                    GL11.glColor4f(color.getRed() / 255.0F, color.getGreen() / 255.0F, color.getBlue() / 255.0F, chams.handOpacity.getValue());
