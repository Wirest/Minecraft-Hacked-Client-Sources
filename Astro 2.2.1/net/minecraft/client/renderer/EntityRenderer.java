...
import dev.astroclient.client.feature.impl.visuals.NoHurtCam;
...
public class EntityRenderer implements IResourceManagerReloadListener {
    ...
    public void getMouseOver(float partialTicks) {
        Reach reach = Client.INSTANCE.featureManager.reach;
        double reachDistance = reach.getState() ? MathUtil.getRandomInRange(reach.minDistance.getValue(), reach.maxDistance.getValue()) : 3D;
        Entity entity = this.mc.getRenderViewEntity();
        ...
        if (entity != null && this.mc.theWorld != null) {
            ...
            for (int i = 0; i < list.size(); ++i) {
                ...
                HitBox hitBox = Client.INSTANCE.featureManager.hitBox;
                float hitboxExpand = hitBox.getState() ? hitBox.expand.getValue() : .1F;
    ...
    private void hurtCameraEffect(float partialTicks) {
        if (((ToggleableFeature)Client.INSTANCE.featureManager.get(NoHurtCam.class)).getState())
            return;
    ...
    private void renderWorldPass(int pass, float partialTicks, long finishTimeNano) {
        ...
        Client.INSTANCE.bus.post(new EventRender3D(partialTicks));
