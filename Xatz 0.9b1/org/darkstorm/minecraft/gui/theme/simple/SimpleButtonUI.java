...
public class SimpleButtonUI extends AbstractComponentUI<Button> {
	...
	protected void renderComponent(Button button) {
		...
		boolean searching = Xatz.getSearchBar() != null && !Xatz.getSearch().isEmpty();
		if (searching) {
			searched = button.getText().toUpperCase().indexOf(Xatz.getSearch().toUpperCase()) != -1;
		}
...
//			if(!Xatz.isModAllowed(bscButton.getModule())) {
...
//		if(button instanceof BasicButton) {
//			BasicButton bscButton = (BasicButton)button;
//			allowed = Xatz.isModAllowed(bscButton.getModule());
//		}
		if(allowed) {
