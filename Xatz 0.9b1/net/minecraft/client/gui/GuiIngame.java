...
import me.xatzdevelopments.xatz.client.modules.Ping;
...
public class GuiIngame extends Gui {
	...
	public void renderGameOverlay(float partialTicks) {
			...
			if (l1 > 8) {
				...
				if (np.isToggled()) {
					for (String s : np.replacements.keySet()) {
						this.title = this.title.replaceAll(s, np.replacements.get(s));
						this.subtitle = this.subtitle.replaceAll(s, np.replacements.get(s));
					}
				}
				GlStateManager.pushMatrix();
				...
				this.getFontRenderer().drawString(this.title,
						(float) (-this.getFontRenderer().getStringWidth(this.title) / 2), -10.0F, 16777215 | j2, true);
				GlStateManager.popMatrix();
				GlStateManager.pushMatrix();
				GlStateManager.scale(2.0F, 2.0F, 2.0F);
				this.getFontRenderer().drawString(this.subtitle,
						(float) (-this.getFontRenderer().getStringWidth(this.subtitle) / 2), 5.0F, 16777215 | j2, true);
				GlStateManager.popMatrix();
				GlStateManager.disableBlend();
				GlStateManager.popMatrix();
			}
		...
		// TODO Atlas uirenderer and onGui
		Xatz.onGui(true);
		GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
...
int ping;
	protected void renderTooltip(ScaledResolution sr, float partialTicks) {
		if(Xatz.getModuleByName("ModernHotbar").isToggled()) {
			...
			Gui.drawRect(0, s.getScaledHeight() - 23, s.getScaledWidth(), s.getScaledHeight(), new Color(0, 0, 0, 180).hashCode());

			if (mc.thePlayer.inventory.currentItem == 0) {
				Gui.drawRect((s.getScaledWidth() / 2) - 91 + mc.thePlayer.inventory.currentItem * 20, s.getScaledHeight() - 23, (s.getScaledWidth() / 2) + 91 - 20 * 8, s.getScaledHeight(), -1);
			} else {
				Gui.drawRect((s.getScaledWidth() / 2) - 91 + mc.thePlayer.inventory.currentItem * 20, s.getScaledHeight() - 23, (s.getScaledWidth() / 2) + 91 - 20 * (8 - mc.thePlayer.inventory.currentItem), s.getScaledHeight(), -1);
			}
			...
			DateTimeFormatter dtf = DateTimeFormatter.ofPattern("HH:mm");  
			   ...
			   GlStateManager.pushMatrix();
			   GlStateManager.translate((s.getScaledWidth() / 1.07), s.getScaledHeight() - 20, 0);
			   GlStateManager.scale(2.2, 2.2, 1);
			   
			int y = 1;
			drawString(getFontRenderer(), dtf.format(now), 0, 0, ColorCreator.createRainbowFromOffset(3200, y  * -15));
			y++;
			GlStateManager.popMatrix();
			if(Xatz.getModuleByName("FPS").isToggled()) {
			drawString(getFontRenderer(), "FPS: " + mc.getDebugFPS(), (s.getScaledWidth() / 230) , s.getScaledHeight() - 9, ColorCreator.createRainbowFromOffset(3200, y  * -15));
			}
			y++;
			if(Xatz.getModuleByName("TPS").isToggled()) {
			drawString(getFontRenderer(), "TPS: " + Math.round(Xatz.lastTps * 10.0) / 10.0, (s.getScaledWidth() / 18), s.getScaledHeight() - 9, ColorCreator.createRainbowFromOffset(3200, y  * -15));
			}
			y++;
			if(Xatz.getModuleByName("Ping").isToggled()) {
			drawString(getFontRenderer(), "Ping: " + ping, (s.getScaledWidth() / 9), s.getScaledHeight() - 9, ColorCreator.createRainbowFromOffset(3200, y  * -15));
			}
			y++;
			if(Xatz.getModuleByName("Coords").isToggled()) {
			drawString(getFontRenderer(), "X: " + (int) mc.thePlayer.posX, (s.getScaledWidth() / 230), s.getScaledHeight() - 20, ColorCreator.createRainbowFromOffset(3200, y  * -15));
			y++;
			drawString(getFontRenderer(), "Y: " + (int) mc.thePlayer.posY, (s.getScaledWidth() / 20), s.getScaledHeight() - 20, ColorCreator.createRainbowFromOffset(3200, y  * -15));
			y++;
			drawString(getFontRenderer(), "Z: " + (int) mc.thePlayer.posZ, (s.getScaledWidth() / 11), s.getScaledHeight() - 20, ColorCreator.createRainbowFromOffset(3200, y  * -15));
			y++;
			}
			//drawString(getFontRenderer(), "height: " + s.getScaledHeight(), 70, 50, ColorCreator.createRainbowFromOffset(3200, y  * -15));
			//drawString(getFontRenderer(), "width: " + s.getScaledWidth(), 70, 60, ColorCreator.createRainbowFromOffset(3200, y  * -15));
		}else {
	...
	private void renderScoreboard(ScoreObjective p_180475_1_, ScaledResolution p_180475_2_) {
		...
		int k = 0;
		// TODO Atlas scoreboard nameprotect
		NameProtect np = (NameProtect) Xatz.getModuleByName("NameProtect");
		for (Score score1 : arraylist1) {
			...
			if (np.isToggled()) {
				for (String s : np.replacements.keySet()) {
					...
					s2 = s2.replaceAll(s, np.replacements.get(s));
				}
			}
			...
			if (k == arraylist1.size()) {
				...
				if (np.isToggled()) {
					for (String s : np.replacements.keySet()) {
						s3 = s3.replaceAll(s, np.replacements.get(s));
					}
				}
