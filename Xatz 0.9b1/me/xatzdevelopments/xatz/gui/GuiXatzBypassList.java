...
public class GuiXatzBypassList extends GuiScreen {
	private int offset = 0;
	private GuiScreen before;

	public GuiXatzBypassList(GuiScreen before) {
		this.before = before;
	}
	...
	public void initGui() {
		...
		for(int i = 0; i < Xatz.getBypassManager().getBypasses().size(); i++) {
			this.buttonList.add(new GuiButton(i + 1, this.width / 2 - 100, height / 2 + (i * 22), 200, 20, Xatz.getBypassManager().getBypasses().get(i).getName()));
		}
	}
	...
	protected void actionPerformed(GuiButton button) throws IOException {
		if (button.id == 0) {
			...
			Xatz.getBypassManager().toggleBypass(button.displayString);
		}
	}
