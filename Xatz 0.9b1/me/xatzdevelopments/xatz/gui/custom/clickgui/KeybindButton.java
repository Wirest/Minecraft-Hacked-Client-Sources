...
import me.xatzdevelopments.xatz.gui.GuiXatzKeyBind;
...
public class KeybindButton extends Component {
	...
	public void draw() {
		...
		GL11.glColor4f(0.2f, 0.2f, 0.2f, 0.8f - Xatz.getClickGuiManager().getAlpha());//keybind button color -masterjunior
		...
		glEnd();
		GL11.glColor4f(0.3f, 0.3f, 0.3f, 0.8f - Xatz.getClickGuiManager().getAlpha());//small line above keybind button -masterjunior
		...
		glEnd();
		GL11.glColor4f(0.1f, 0.1f, 0.1f, 0.8f - Xatz.getClickGuiManager().getAlpha());//small line below keybind button -masterjunior
		glBegin(GL11.GL_LINES);
		{
		...
		if(mod != null && Xatz.getClickGuiManager().getAlpha() <= 0.5f) {
			fontRenderer.drawString(title, (int) (getWidth() / 2 - fontRenderer.getStringWidth(title) / 2), 0, 
					0xffffffff);//keybind text color -masterjunior
		}
	...
	public void onClicked(double x, double y, int button) {
		...
		if(button == 0) {
			...
			.displayGuiScreen(new GuiXatzKeyBind(mod, Minecraft.getMinecraft().currentScreen));
		}
	}
