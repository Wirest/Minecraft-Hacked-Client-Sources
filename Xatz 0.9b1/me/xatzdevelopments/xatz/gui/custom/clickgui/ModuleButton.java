...
public class ModuleButton extends Component {
	...
	public void draw() {
		...
		GL11.glColor4f(0.15f, 0.15f, 0.15f, 1f - Xatz.getClickGuiManager().getAlpha());//untoggled modules button -masterjunior
		if(mod.isToggled()) {
		...
		glEnd();
		if(hovered) {
			GL11.glColor4f(0.5f, 0.5f, 0.5f, 0.2f - Xatz.getClickGuiManager().getAlpha());//hovered module color -masterjunior
			glBegin(GL11.GL_QUADS);
			{
				glVertex2d(0, 0);
				glVertex2d(0, getHeight());
		...
		if(mod != null && Xatz.getClickGuiManager().getAlpha() <= 0.5f) {
			fontRenderer.drawStringWithShadow(title, (float) (getWidth() / 2 - fontRenderer.getStringWidth(title) / 2), 1, 0xffffffff);//module name color - masterjunior
		}
