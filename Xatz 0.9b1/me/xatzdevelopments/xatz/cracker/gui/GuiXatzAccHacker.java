package me.xatzdevelopments.xatz.cracker.gui;
...
public class GuiXatzAccHacker extends GuiScreen {

	private GuiScreen before;
	private GuiTextField emailField;
	private String status = "idle";

	public GuiXatzAccHacker(GuiScreen before) {
		this.before = before;
	}
	...
	public void initGui() {
		...
		if (Xatz.getCrackManager() != null && Xatz.getCrackManager().isCracking) {
	...
	protected void actionPerformed(GuiButton button) throws IOException {
		...
		if(button.enabled) {
			if (id == 0) {
				Xatz.setCrackManager(new CrackManager());
				Xatz.getCrackManager().initLists();
				Xatz.getCrackManager().startCracking(emailField.getText());
			...
			if (id == 2) {
				Xatz.getCrackManager().stop = true;
				Xatz.getCrackManager().isCracking = false;
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		if (Xatz.getCrackManager() != null) {
			drawString(fontRendererObj, "ℤ-§7Tries: " + Xatz.getCrackManager().tries, 10, 20, 0xffffffff);
			drawString(fontRendererObj, "ℤ-§7Bad Proxies: " + Xatz.getCrackManager().badProxies, 10, 30, 0xffffffff);
			drawString(fontRendererObj, "ℤ-§7Credentials: "
					+ (Xatz.getCrackManager().emailPass == null ? "unknown" : Xatz.getCrackManager().emailPass), 10,
					40, 0xffffffff);
		}
		GlStateManager.scale(4, 4, 1);
		drawCenteredString(fontRendererObj, Xatz.headerNoBrackets, this.width / 2 / 4, (this.height / 2 - 67 - 25) / 4, 0xffffffff);
	...
	public void updateScreen() {
		this.status = (Xatz.getCrackManager() != null && Xatz.getCrackManager().isCracking) ? "Cracking..."
				: "idle";
