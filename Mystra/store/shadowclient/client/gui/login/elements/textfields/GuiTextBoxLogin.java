...
public class GuiTextBoxLogin extends GuiTextField {
	...
	public void drawTextBox(int mouseX, int mouseY) {
		Shadow.RENDER2D.rect(xPosition,  yPosition, width, height, color);
		...
		if (this.cursorCounter / 6 % 2 == 0 && isFocused()) {
			if (cursorPosition == getTextUntrimmed().length()) {
				Shadow.fontManager.getFont("SFL 10").drawStringWithShadow("_", xPosition + 4 - offsetX + Shadow.fontManager.getFont("SFL 10").getWidth((hide ? getText().replaceAll(".", "*") : getText())), yPosition + height / 2 - Shadow.fontManager.getFont("SFL 10").getHeight("_") / 2, -1);
			}
		}
