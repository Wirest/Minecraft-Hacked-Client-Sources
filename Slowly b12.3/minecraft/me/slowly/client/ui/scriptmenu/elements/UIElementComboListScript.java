...
public class UIElementComboListScript {
   ...
   public UIElementComboListScript() {
   ...
   public void draw(float xPosition, float yPosition, int mouseX, int mouseY) {
      ...
      for(int i = 0; i < this.options.size(); ++i) {
         ...
         GL11.glPushMatrix();
         GL11.glEnable(3089);
         RenderUtil.doGlScissor(this.x, yAxis, this.width, this.height);
         this.font.drawString(((Script)this.options.get(i)).getName(), (float)(this.x + 1), (float)yAxis + (float)(this.height - this.font.FONT_HEIGHT) / 2.0F, -2894893);
         GL11.glDisable(3089);
         GL11.glPopMatrix();
         yAxis += this.height;
         this.font = Client.getInstance().getFontManager().consolas14;
   ...
   public void moveSelectedUp() {
      if (this.canMoveUp()) {
         Script swap = (Script)this.options.get(this.selected);
         this.options.set(this.selected, (Script)this.options.get(this.selected - 1));
   ...
   public Script getSelected() {
      return this.canDelete() ? (Script)this.options.get(this.selected) : null;
   }
