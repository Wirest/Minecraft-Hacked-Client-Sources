...
public class ModButton {
   ...
   public ModButton(Mod.Category cat, int x, int y, int width, int height, int panelY, int yAdd) {
   ...
   public void drawOutlinedRect(int x, int y, int width, int height, float lineSize, Color lineColor, Color backgroundColor) {
      Gui.drawRect(x, y, width, height, backgroundColor.getRGB());
      Gui.drawRect((float)x, (float)y, (float)width, (float)y + lineSize, lineColor.getRGB());
      Gui.drawRect((float)x, (float)height - lineSize, (float)width, (float)height, lineColor.getRGB());
      Gui.drawRect((float)x, (float)y + lineSize, (float)x + lineSize, (float)height - lineSize, lineColor.getRGB());
      Gui.drawRect((float)width - lineSize, (float)y + lineSize, (float)width, (float)height - lineSize, lineColor.getRGB());
   }
   ...
   public void drawScreen(int mouseX, int mouseY, float partialTicks, float delta) {
      ...
      while(true) {
         ...
         if (this.y + this.yAxis >= this.panelY) {
            this.drawOutlinedRect(this.x + 1, this.y + this.yAxis, this.x + this.width - 1, this.y + this.yAxis + this.height, 0.1F, Color.WHITE, new Color(-16777215));
            int xOld = this.x;
            int yOld = this.y;
            ...
            if (this.isOpen) {
               if (this.isHovering(mouseX, mouseY, this.x + 1, this.y + this.yAxis, this.x + this.width - 1, this.y + this.yAxis + this.height)) {
                  UIOther.currentMod = m;
                  hover = true;
               }

               if (UIOther.currentMod == m && this.yAdd == 0.0F) {
            ...
            Client.getInstance().getFontManager().simpleton17.drawCenteredString(m.getName(), (float)this.x + halfWidth, (float)this.y + halfHeight - stringHeight / 2.0F + (float)this.yAxis + 3.0F, m.isEnabled() ? -10785921 : -1);
            if (m.openValues) {
               ...
               while(true) {
                  ...
                  if (value.isValueMode) {
                     ...
                     for(int i = 0; i < value.listModes().size(); ++i) {
                        ...
                        Gui.drawRect(this.x + 1, this.y + this.yAxis, this.x + this.width - 1, this.y + this.yAxis + this.height, -16777215);
                        ...
                        if (value.getCurrentMode() == i) {
                           ...
                           Client.getInstance().getFontManager().simpleton16.drawString(name, var10002, var10003 - (float)stringHeightMode / 2.0F, -14207162);
                        } else {
                           var10002 = (float)this.x + (float)this.width / 2.0F - (float)stringWidthMode / 2.0F;
                           var10003 = (float)(this.y + this.yAxis) + (float)this.height / 2.0F;
                  ...
                  if (value.isValueBoolean) {
                     ...
                     Client.getInstance().getFontManager().simpleton16.drawCenteredString(valueName, var10002, var10003 + (float)this.yAxis, ((Boolean)value.getValueState()).booleanValue() ? -14207162 : -1);
                     if (Mouse.isButtonDown(0) && !value.disabled) {
