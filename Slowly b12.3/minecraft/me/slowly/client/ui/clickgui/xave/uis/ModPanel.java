package me.slowly.client.ui.clickgui.xave.uis;
...
public class ModPanel {
   ...
   public ModPanel(Mod.Category cat, int x, int y, int width, int height, int panelY, int yAdd) {
   ...
   public void drawScreen(int mouseX, int mouseY, float partialTicks, float delta) {
      ...
      while(true) {
         ...
         if (this.y + this.yAxis >= this.panelY) {
            Gui.drawRect(this.x + 1, this.y + this.yAxis, this.x + this.width - 1, this.y + this.yAxis + this.height, new Color(0, 0, 0, 200));
            int xOld = this.x;
            int yOld = this.y;
            ...
            if (this.isOpen) {
               ...
               if (UIXave.currentMod == m && this.yAdd == 0.0F) {
            ...
            Client.getInstance().getFontManager().sant15.drawCenteredString(m.getName(), (float)this.x + halfWidth, (float)this.y + halfHeight - stringHeight / 2.0F + (float)this.yAxis + 1.0F, m.isEnabled() ? -14130797 : -10919065);
            if (m.openValues) {
               ...
               while(true) {
                  ...
                  if (value.isValueMode) {
                     ...
                     for(int i = 0; i < value.listModes().size(); ++i) {
                        ...
                        if (value.getCurrentMode() == i) {
                           ...
                           Client.getInstance().getFontManager().sant15.drawString(name, var10002, var10003 - (float)stringHeightMode / 2.0F, -14130797);
                        } else {
                           var10002 = (float)this.x + (float)this.width / 2.0F - (float)stringWidthMode / 2.0F;
                           var10003 = (float)(this.y + this.yAxis) + (float)this.height / 2.0F;
                           Client.getInstance().getFontManager().sant15.drawString(name, var10002, var10003 - (float)stringHeightMode / 2.0F, -1);
                        }
                  ...
                  if (value.isValueBoolean) {
                     Gui.drawRect(this.x + 1, this.y + this.yAxis, this.x + this.width - 1, this.y + this.yAxis + this.height, new Color(0, 0, 0, 200));
                     var10002 = (float)(this.x + 4);
                     var10003 = (float)this.y + halfHeight - valueStringHeight / 2.0F;
                     Client.getInstance().getFontManager().sant15.drawString("x", var10002, var10003 + (float)this.yAxis, ((Boolean)value.getValueState()).booleanValue() ? -14057287 : -1);
                     var10002 = (float)this.x + halfWidth;
                     var10003 = (float)this.y + halfHeight - valueStringHeight / 2.0F + (float)this.yAxis;
                     Client.getInstance().getFontManager().sant15.drawCenteredString(valueName, var10002, var10003, -1);
                     if (Mouse.isButtonDown(0) && !value.disabled) {
                  ...
                  if (value.isValueDouble) {
                     ...
                     Gui.drawRect(this.x + 1, this.y + this.yAxis, this.x + this.width - 1, this.y + this.yAxis + this.height, new Color(0, 0, 0, 200));
                     Gui.drawRect(this.x + 1, this.y + this.yAxis + 1, this.x + this.width - 1, this.y + this.yAxis + this.height - 2, new Color(0, 0, 0, 250));
                     GuiIngame var41 = Minecraft.getMinecraft().ingameGUI;
                     GuiIngame.drawRect((float)(this.x + 1), (float)(this.y + this.yAxis + 1), (float)val, (float)(this.y + this.yAxis + this.height - 2), -14057287);
                     ...
                     Client.getInstance().getFontManager().sant15.drawString(String.valueOf(valueName) + " " + value.getValueState(), (float)this.x + halfWidth - valueStringWidthDouble / 2.0F, (float)this.y + halfHeight - valueStringHeight / 2.0F + (float)this.yAxis, -1);
                     if (value.disabled) {
