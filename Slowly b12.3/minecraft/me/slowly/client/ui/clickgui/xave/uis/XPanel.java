...
public class XPanel {
   ...
   private ModPanel modulePanel;
   private int openValueY;
   private int allModules;
   private static final float SPEED = 100.0F;
   private float yAdd;

   public XPanel(int x, int y, Mod.Category cat) {
      this.yAdd = (float)(-this.width);
      this.x = x;
      this.y = y;
      this.cat = cat;
      this.modulePanel = new ModPanel(cat, x * 2, y, this.width, this.width, y, 0);
   ...
   public void drawScreen(int mouseX, int mouseY, float partialTicks) {
      ...
      GuiIngame.drawRect(this.x - 2, this.y, this.x + this.width + 2, this.y + this.height + 1, Colors.BLACK.c);
      var10000 = Minecraft.getMinecraft().ingameGUI;
      GuiIngame.drawRect(this.x - 2, this.y, this.x + this.width + 2, this.y + this.height, -13987361);
      Client.getInstance().getFontManager().sant15.drawString(this.cat.name(), (float)this.x + (float)this.width / 2.0F - this.getStringWidth() / 2.0F, (float)this.y + (float)this.height / 2.0F - this.getStringHeight() / 2.0F + 1.0F, -1);
