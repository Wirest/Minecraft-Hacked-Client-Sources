...
public class GuiIngameMenu extends GuiScreen {
   ...
   public void initGui() {
      ...
      this.buttonList.add(new GuiButton(1337, this.width / 2 + 2, this.height / 4 + 120 + i, 98, 20, "Reconnect"));
   ...
   protected void actionPerformed(GuiButton button) throws IOException {
      ...
      case 1337:
         ServerData oldData = this.mc.getCurrentServerData();
         boolean flag_ = this.mc.isIntegratedServerRunning();
         boolean flag1_ = this.mc.func_181540_al();
         button.enabled = false;
         this.mc.theWorld.sendQuittingDisconnectingPacket();
         this.mc.loadWorld((WorldClient)null);
         if (flag_) {
            this.mc.displayGuiScreen(new GuiConnecting(new GuiMultiplayer((GuiScreen)null), this.mc, oldData));
         } else if (flag1_) {
            RealmsBridge realmsbridge = new RealmsBridge();
            realmsbridge.switchToRealms(new GuiMainMenu());
         } else {
