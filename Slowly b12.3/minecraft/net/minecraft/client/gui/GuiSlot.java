...
import me.slowly.client.shaders.fragment.BackGroundShader;
...
public abstract class GuiSlot {
   public static BackGroundShader backgroundShader;
   ...
   private void drawWorldBackground(int tint) {
      if (this.mc.theWorld != null) {
         Gui.drawRect(0, 0, this.width, this.height, ClientUtil.reAlpha(FlatColors.DARK_BLUE.c, 0.25F));
      } else {
   ...
   public void drawScreen(int mouseXIn, int mouseYIn, float p_148128_3_) {
      ...
      int k1;
      int l1;
      if ((this.mc.currentScreen instanceof GuiMultiplayer || this.mc.currentScreen instanceof GuiSelectWorld) && !this.mc.gameSettings.ofFastRender) {
         if (this.field_178041_q) {
            ...
            if (this.mc.gameSettings.ofFastRender) {
               ...
               backgroundShader.startShader();
               RenderUtil.drawImage(new ResourceLocation("slowly/flat2.jpg"), 0, 0, res.getScaledWidth(), res.getScaledHeight());
               backgroundShader.stopShader();
            }

            if (!this.loaded && !this.mc.gameSettings.ofFastRender) {
               backgroundShader.deleteShader();
               this.loaded = true;
               this.timer.reset();
            }
            ...
            WorldRenderer worldrenderer = tessellator.getWorldRenderer();
            GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
            k = this.left + this.width / 2 - this.getListWidth() / 2 + 2;
            l = this.top + 4 - (int)this.amountScrolled;
            ...
            tessellator.draw();
            j1 = this.func_148135_f();
            if (j1 > 0) {
               k1 = (this.bottom - this.top) * (this.bottom - this.top) / this.getContentHeight();
               k1 = MathHelper.clamp_int(k1, 32, this.bottom - this.top - 8);
               l1 = (int)this.amountScrolled * (this.bottom - this.top - k1) / j1 + this.top;
               if (l1 < this.top) {
                  l1 = this.top;
               }
      ...
      } else if (this.field_178041_q) {
         this.mouseX = mouseXIn;
         this.mouseY = mouseYIn;
         this.drawBackground();
         int i11 = this.getScrollBarX();
         i11 = i11 + 6;
         this.bindAmountScrolled();
         GlStateManager.disableLighting();
         GlStateManager.disableFog();
         ...
         GlStateManager.disableTexture2D();
         worldrenderer.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
         worldrenderer.pos((double)this.left, (double)(this.top + i11), 0.0D).tex(0.0D, 1.0D).color(0, 0, 0, 0).endVertex();
         worldrenderer.pos((double)this.right, (double)(this.top + i11), 0.0D).tex(1.0D, 1.0D).color(0, 0, 0, 0).endVertex();
         worldrenderer.pos((double)this.right, (double)this.top, 0.0D).tex(1.0D, 0.0D).color(0, 0, 0, 255).endVertex();
         worldrenderer.pos((double)this.left, (double)this.top, 0.0D).tex(0.0D, 0.0D).color(0, 0, 0, 255).endVertex();
         tessellator.draw();
         worldrenderer.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
         worldrenderer.pos((double)this.left, (double)this.bottom, 0.0D).tex(0.0D, 1.0D).color(0, 0, 0, 255).endVertex();
         worldrenderer.pos((double)this.right, (double)this.bottom, 0.0D).tex(1.0D, 1.0D).color(0, 0, 0, 255).endVertex();
         worldrenderer.pos((double)this.right, (double)(this.bottom - i11), 0.0D).tex(1.0D, 0.0D).color(0, 0, 0, 0).endVertex();
         worldrenderer.pos((double)this.left, (double)(this.bottom - i11), 0.0D).tex(0.0D, 0.0D).color(0, 0, 0, 0).endVertex();
         tessellator.draw();
         j1 = this.func_148135_f();
         if (j1 > 0) {
            k1 = (this.bottom - this.top) * (this.bottom - this.top) / this.getContentHeight();
            ...
            worldrenderer.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
            worldrenderer.pos((double)i11, (double)this.bottom, 0.0D).tex(0.0D, 1.0D).color(0, 0, 0, 255).endVertex();
            worldrenderer.pos((double)i11, (double)this.bottom, 0.0D).tex(1.0D, 1.0D).color(0, 0, 0, 255).endVertex();
            worldrenderer.pos((double)i11, (double)this.top, 0.0D).tex(1.0D, 0.0D).color(0, 0, 0, 255).endVertex();
            worldrenderer.pos((double)i11, (double)this.top, 0.0D).tex(0.0D, 0.0D).color(0, 0, 0, 255).endVertex();
            tessellator.draw();
            worldrenderer.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
            worldrenderer.pos((double)i11, (double)(l1 + k1), 0.0D).tex(0.0D, 1.0D).color(128, 128, 128, 255).endVertex();
            worldrenderer.pos((double)i11, (double)(l1 + k1), 0.0D).tex(1.0D, 1.0D).color(128, 128, 128, 255).endVertex();
            worldrenderer.pos((double)i11, (double)l1, 0.0D).tex(1.0D, 0.0D).color(128, 128, 128, 255).endVertex();
            worldrenderer.pos((double)i11, (double)l1, 0.0D).tex(0.0D, 0.0D).color(128, 128, 128, 255).endVertex();
            tessellator.draw();
            worldrenderer.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
            worldrenderer.pos((double)i11, (double)(l1 + k1 - 1), 0.0D).tex(0.0D, 1.0D).color(192, 192, 192, 255).endVertex();
            worldrenderer.pos((double)(i11 - 1), (double)(l1 + k1 - 1), 0.0D).tex(1.0D, 1.0D).color(192, 192, 192, 255).endVertex();
            worldrenderer.pos((double)(i11 - 1), (double)l1, 0.0D).tex(1.0D, 0.0D).color(192, 192, 192, 255).endVertex();
            worldrenderer.pos((double)i11, (double)l1, 0.0D).tex(0.0D, 0.0D).color(192, 192, 192, 255).endVertex();
            tessellator.draw();
         }
