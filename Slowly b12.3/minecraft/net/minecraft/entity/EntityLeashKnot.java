...
public class EntityLeashKnot extends EntityHanging
{
    ...
    public boolean interactFirst(EntityPlayer playerIn) {
        ...
        if (itemstack != null && itemstack.getItem() == Items.lead && !this.worldObj.isRemote) {
           d1 = 7.0D;
           var7 = this.worldObj.getEntitiesWithinAABB(EntityLiving.class, new AxisAlignedBB(this.posX - d1, this.posY - d1, this.posZ - d1, this.posX + d1, this.posY + d1, this.posZ + d1)).iterator();
    ...
    public static EntityLeashKnot getKnotForPosition(World worldIn, BlockPos pos) {
        ...
        int j = pos.getY();
        int k = pos.getZ();
        Iterator var6 = worldIn.getEntitiesWithinAABB(EntityLeashKnot.class, new AxisAlignedBB((double)i - 1.0D, (double)j - 1.0D, (double)k - 1.0D, (double)i + 1.0D, (double)j + 1.0D, (double)k + 1.0D)).iterator();

        while(var6.hasNext()) {
           EntityLeashKnot entityleashknot = (EntityLeashKnot)var6.next();
