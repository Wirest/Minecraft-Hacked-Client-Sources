...
public class ItemLead extends Item
{
    ...
    public static boolean attachToFence(EntityPlayer player, World worldIn, BlockPos fence) {
        ...
        int j = fence.getY();
        int k = fence.getZ();
        Iterator var11 = worldIn.getEntitiesWithinAABB(EntityLiving.class, new AxisAlignedBB((double)i - d0, (double)j - d0, (double)k - d0, (double)i + d0, (double)j + d0, (double)k + d0)).iterator();

        while(var11.hasNext()) {
           EntityLiving entityliving = (EntityLiving)var11.next();
