...
public final class Header {
   ...
   void parseVBR(byte[] firstframe) throws BitstreamException {
      ...
      try {
         System.arraycopy(firstframe, offset1, tmp, 0, 4);
         if (xing.equals(new String(tmp))) {
            ...
            System.arraycopy(firstframe, offset1 + length, flags, 0, flags.length);
            int length1 = 4 + flags.length;
            if ((flags[3] & 1) != 0) {
               System.arraycopy(firstframe, offset1 + length1, tmp, 0, tmp.length);
            ...
            if ((flags[3] & 4) != 0) {
               System.arraycopy(firstframe, offset1 + length1, this.h_vbr_toc, 0, this.h_vbr_toc.length);
               length1 += this.h_vbr_toc.length;
            }
      ...
      try {
         ...
         if (vbri.equals(new String(tmp))) {
            ...
            this.h_vbr_toc = new byte[100];
            int length = 10;
            System.arraycopy(firstframe, offset1 + length, tmp, 0, tmp.length);
            this.h_vbr_bytes = tmp[0] << 24 & -16777216 | tmp[1] << 16 & 16711680 | tmp[2] << 8 & '\uff00' | tmp[3] & 255;
            int length1 = 4 + 4;
            System.arraycopy(firstframe, offset1 + length1, tmp, 0, tmp.length);
            this.h_vbr_frames = tmp[0] << 24 & -16777216 | tmp[1] << 16 & 16711680 | tmp[2] << 8 & '\uff00' | tmp[3] & 255;
