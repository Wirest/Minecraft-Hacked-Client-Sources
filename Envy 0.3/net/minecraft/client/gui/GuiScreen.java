...
import me.ihaq.iClient.gui.clickgui.ClickGui;
...
public abstract class GuiScreen extends Gui implements GuiYesNoCallback {
	...
	public ParticleSystem ps = new ParticleSystem(150);
	...
	public void drawBackground(int tint) {
		 ...
		 * GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F); float f = 32.0F;
		 * worldrenderer.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
		 * worldrenderer.pos(0.0D, (double)this.height, 0.0D).tex(0.0D,
		 * (double)((float)this.height / 32.0F + (float)tint)).color(64, 64, 64,
		 * 255).endVertex(); worldrenderer.pos((double)this.width,
		 * (double)this.height, 0.0D).tex((double)((float)this.width / 32.0F),
		 * (double)((float)this.height / 32.0F + (float)tint)).color(64, 64, 64,
		 * 255).endVertex(); worldrenderer.pos((double)this.width, 0.0D,
		 * 0.0D).tex((double)((float)this.width / 32.0F),
		 * (double)tint).color(64, 64, 64, 255).endVertex();
		 * worldrenderer.pos(0.0D, 0.0D, 0.0D).tex(0.0D, (double)tint).color(64,
		 * 64, 64, 255).endVertex(); tessellator.draw();
		 */
