...
import me.CheerioFX.FusionX.events.EventDamage;
import me.CheerioFX.FusionX.events.JumpEvent;
...
public abstract class EntityPlayer extends EntityLivingBase {
	...
	protected void updateItemUse(ItemStack itemStackIn, int p_71010_2_) {
		...
		if (itemStackIn.getItemUseAction() == EnumAction.EAT) {
			...
			this.playSound("random.eat", 0.5F + 0.5F * this.rand.nextInt(2),
	...
	public EntityItem func_146097_a(ItemStack p_146097_1_, boolean p_146097_2_, boolean p_146097_3_) {
		if (p_146097_1_ == null) {
			...
			if (p_146097_2_) {
				...
				var7 = 0.3F;
				var6.motionX = -MathHelper.sin(this.rotationYaw / 180.0F * (float) Math.PI)
						* MathHelper.cos(this.rotationPitch / 180.0F * (float) Math.PI) * var7;
				var6.motionZ = MathHelper.cos(this.rotationYaw / 180.0F * (float) Math.PI)
						* MathHelper.cos(this.rotationPitch / 180.0F * (float) Math.PI) * var7;
	...
	protected void damageEntity(final DamageSource p_70665_1_, float p_70665_2_) {
		final EventDamage event = new EventDamage();
					...
					if (var14) {
						if (var18 > 0) {
							targetEntity.addVelocity(
									-MathHelper.sin(this.rotationYaw * (float) Math.PI / 180.0F) * var18 * 0.5F, 0.1D,
									MathHelper.cos(this.rotationYaw * (float) Math.PI / 180.0F) * var18 * 0.5F);
							this.motionX *= 0.6D;
							this.motionZ *= 0.6D;
							if (!Sprint.isSprintEnabled()) {
	...
	public EntityPlayer.EnumStatus func_180469_a(BlockPos p_180469_1_) {
		if (!this.worldObj.isRemote) {
			...
			if (Math.abs(this.posX - p_180469_1_.getX()) > 3.0D || Math.abs(this.posY - p_180469_1_.getY()) > 2.0D
					|| Math.abs(this.posZ - p_180469_1_.getZ()) > 3.0D) {
		...
		if (this.worldObj.isBlockLoaded(p_180469_1_)) {
			...
			this.setPosition(p_180469_1_.getX() + var3, p_180469_1_.getY() + 0.6875F, p_180469_1_.getZ() + var8);
		} else {
