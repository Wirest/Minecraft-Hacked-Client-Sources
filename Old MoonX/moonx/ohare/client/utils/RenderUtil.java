...
public class RenderUtil {
    ...
    public static void renderTag(String name, double pX, double pY, double pZ, int color) {
        float scale = (float) (mc.thePlayer.getDistance(pX + mc.getRenderManager().getRenderPosX(), pY + mc.getRenderManager().getRenderPosY(), pZ + mc.getRenderManager().getRenderPosZ()) / 8.0D);
        if (scale < 1.6F) {
            scale = 1.6F;
        }
        scale /= 50;
