...
public class Panel {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		if(Cheatware.instance.settingsManager.getSettingByName("Design").getValString().equalsIgnoreCase("New")){
			FontUtil.drawStringWithShadow(title, x + 21, y + height / 2 - FontUtil.getFontHeight()/2, RainbowUtil.rainbow());
		}else if(Cheatware.instance.settingsManager.getSettingByName("Design").getValString().equalsIgnoreCase("JellyLike")){
			Gui.drawRect(x + 4,			y + 2, x + 4.3, 		y + height - 2, 0xff000000);
			Gui.drawRect(x - 4 + width, y + 2, x - 4.3 + width, y + height - 2, 0xff000000);
			FontUtil.drawTotalCenteredStringWithShadow(title, x + width / 2, y + height / 2, 0xffefefef);
		}
		
		if (this.extended && !Elements.isEmpty()) {
			double startY = y + height;
			int epanelcolor = Cheatware.instance.settingsManager.getSettingByName("Design").getValString().equalsIgnoreCase("New") ? 0xff232323 : Cheatware.instance.settingsManager.getSettingByName("Design").getValString().equalsIgnoreCase("JellyLike") ? 0xbb151515 : 0;;
			for (ModuleButton et : Elements) {
				
				Gui.drawRect(x, 	startY, x + width, startY + et.height + 1, 0x90ffffff);
				et.x = x + 2;
				et.y = startY;
				et.width = width - 4;
