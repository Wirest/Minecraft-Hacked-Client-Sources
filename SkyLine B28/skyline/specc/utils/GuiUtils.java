...
public class GuiUtils extends Gui
{
    ...
    public static void drawCheck(final float x, final float y, final int color) {
        Mineman.getMinecraft().fontRendererObj.drawString("\u2713", x, y, color);
        Mineman.getMinecraft().fontRendererObj.drawString("\u2713", x, y + 0.5f, color);
    }
    ...
    public void drawTexturedRectangle(final ResourceLocation texture, final float x, final float y, final float width, final float height, final float u, final float v, final float uWidth, final float vHeight) {
        GL11.glEnable(3042);
        Mineman.getMinecraft().getTextureManager().bindTexture(texture);
    ...
    public static void scissorBox(final int x, final int y, final int xend, final int yend) {
        ...
        final ScaledResolution sr = new ScaledResolution(Mineman.getMinecraft(), Mineman.getMinecraft().displayWidth, Mineman.getMinecraft().displayHeight);
        final int factor = sr.getScaleFactor();
        final int bottomY = Mineman.getMinecraft().currentScreen.height - yend;
    ...
    public void draw2DPlayerESP(final EntityPlayer ep, final double d, final double d1, final double d2) {
        final float distance = Mineman.getMinecraft().thePlayer.getDistanceToEntity(ep);
        final float scale = (float)(0.09 + Mineman.getMinecraft().thePlayer.getDistance(ep.posX, ep.posY, ep.posZ) / 10000.0);
        GL11.glPushMatrix();
