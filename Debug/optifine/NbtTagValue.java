...
public class NbtTagValue
{
    ...
    public NbtTagValue(String p_i77_1_, String p_i77_2_)
    {
        String[] astring = Config.tokenize(p_i77_1_, ".");
        this.parents = (String[])Arrays.copyOfRange(astring, 0, astring.length - 1);
        this.name = astring[astring.length - 1];

        if (p_i77_2_.startsWith("!"))
        {
            this.negative = true;
            p_i77_2_ = p_i77_2_.substring(1);
        }

        if (p_i77_2_.startsWith("pattern:"))
        {
            this.type = 1;
            p_i77_2_ = p_i77_2_.substring("pattern:".length());
        }
        else if (p_i77_2_.startsWith("ipattern:"))
        {
            this.type = 2;
            p_i77_2_ = p_i77_2_.substring("ipattern:".length()).toLowerCase();
        }
        else if (p_i77_2_.startsWith("regex:"))
        {
            this.type = 3;
            p_i77_2_ = p_i77_2_.substring("regex:".length());
        }
        else if (p_i77_2_.startsWith("iregex:"))
        {
            this.type = 4;
            p_i77_2_ = p_i77_2_.substring("iregex:".length()).toLowerCase();
        }
        else
        {
            this.type = 0;
        ...
        p_i77_2_ = StringEscapeUtils.unescapeJava(p_i77_2_);

        if (this.type == 0 && PATTERN_HEX_COLOR.matcher(p_i77_2_).matches())
        {
            this.valueFormat = 1;
        }
    ...
    {
        return this.negative ? !this.matchesCompound(p_matches_1_) : this.matchesCompound(p_matches_1_);
    }
