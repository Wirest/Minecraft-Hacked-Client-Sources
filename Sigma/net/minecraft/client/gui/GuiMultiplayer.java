...
public class GuiMultiplayer extends GuiScreen implements GuiYesNoCallback {
    ...
    public void connectToSelected() {
        ...
        if (((ServerListEntryNormal) var1).getServerData().serverIP.toLowerCase().contains("hypixel")) {
            Notifications.getManager().post("Auto Config", "Use .config load hypixel for premade settings!", 5000, Notifications.Type.WARNING);
            ...
            Notifications.getManager().post("Auto Config", "Use .config load mineplex for premade settings!", 5000, Notifications.Type.WARNING);
            if (!Client.getModuleManager().isEnabled(AntiBot.class)) {
                Client.getModuleManager().get(AntiBot.class).toggle();
                Notifications.getManager().post("Auto Config", "AntiBot has been enabled for your protection.", Notifications.Type.WARNING);
            }
            Module antiBot = Client.getModuleManager().get(AntiBot.class);
            if (antiBot.getSetting(AntiBot.MODE) != null && !((Options) antiBot.getSetting(AntiBot.MODE).getValue()).getSelected().equals("Mineplex")) {
                ((Options) antiBot.getSetting(AntiBot.MODE).getValue()).setSelected("Mineplex");
                Notifications.getManager().post("AntiBot", "Switched to Mineplex for your protection.", Notifications.Type.NOTIFY);
            }
        } else if (((ServerListEntryNormal) var1).getServerData().serverIP.toLowerCase().contains("cubecraft")) {
            Notifications.getManager().post("Auto Config", "Use .config load cubecraft for premade settings!", 5000, Notifications.Type.WARNING);
