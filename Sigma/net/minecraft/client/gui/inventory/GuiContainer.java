...
public abstract class GuiContainer extends GuiScreen {
    ...
    public void initGui() {
        ...
        this.buttonList.add(new GuiButton(696969, res.getScaledWidth() / 2 - 115, 3, 110, 20, "Inventory Manager"));
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    	...
    	if((Boolean) Client.getModuleManager().get(Enabled.class).getSetting(Enabled.CONTAINER).getValue()){
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        for (Object aButtonList : this.buttonList) {
            ...
            if (var52.mousePressed(this.mc, mouseX, mouseY) && var52.id == 6969) {
            	if(Client.getModuleManager().isEnabled(Killaura.class))
            		Client.getModuleManager().get(Killaura.class).toggle();
            	if(Client.getModuleManager().isEnabled(InfiniteAura.class))
                    Client.getModuleManager().get(InfiniteAura.class).toggle();
            }
            if (var52.mousePressed(this.mc, mouseX, mouseY) && var52.id == 696969) {
            	this.mc.displayGuiScreen(new GuiInvManager());
            }
        }
