...
public class NetHandlerPlayClient implements INetHandlerPlayClient {
    ...
    public void handlePlayerPosLook(S08PacketPlayerPosLook packetIn) {
        ...
        if (packetIn.getSet().contains(S08PacketPlayerPosLook.EnumFlags.X)) {
            var3 += var2.posX;
        } else {
            var2.motionX = 0.0D;
        }

        if (packetIn.getSet().contains(S08PacketPlayerPosLook.EnumFlags.Y)) {
            var5 += var2.posY;
        } else {
            var2.motionY = 0.0D;
        }

        if (packetIn.getSet().contains(S08PacketPlayerPosLook.EnumFlags.Z)) {
            var7 += var2.posZ;
        } else {
            var2.motionZ = 0.0D;
        }

        if (packetIn.getSet().contains(S08PacketPlayerPosLook.EnumFlags.X_ROT)) {
            var10 += var2.rotationPitch;
        }

        if (packetIn.getSet().contains(S08PacketPlayerPosLook.EnumFlags.Y_ROT)) {
    ...
    public void onDisconnect(IChatComponent reason) {
        ...
        if (this.guiScreenServer != null) {
            if (this.guiScreenServer instanceof GuiScreenRealmsProxy) {
            	...
            	GuiDisconnected gui = new GuiDisconnected(this.guiScreenServer, "disconnect.lost", reason);
            	gui.setIp(ip);
                this.gameController.displayGuiScreen(gui);
            }
        } else {
        	GuiDisconnected gui = new GuiDisconnected(new GuiMultiplayer(new ClientMainMenu()), "disconnect.lost", reason);
        	gui.setIp(ip);
            this.gameController.displayGuiScreen(gui);
        }
    }
