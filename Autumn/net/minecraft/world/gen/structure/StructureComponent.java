...
public abstract class StructureComponent {
   ...
   protected boolean isLiquidInStructureBoundingBox(World worldIn, StructureBoundingBox boundingboxIn) {
      ...
      for(j2 = i; j2 <= l; ++j2) {
         for(l2 = k; l2 <= j1; ++l2) {
            if (worldIn.getBlockState(blockpos$mutableblockpos.func_181079_c(j2, j, l2)).getBlock().getMaterial().isLiquid()) {
               return true;
            }

            if (worldIn.getBlockState(blockpos$mutableblockpos.func_181079_c(j2, i1, l2)).getBlock().getMaterial().isLiquid()) {
               return true;
            }
         }
      }
      ...
      for(j2 = i; j2 <= l; ++j2) {
         for(l2 = j; l2 <= i1; ++l2) {
            if (worldIn.getBlockState(blockpos$mutableblockpos.func_181079_c(j2, l2, k)).getBlock().getMaterial().isLiquid()) {
               return true;
            }

            if (worldIn.getBlockState(blockpos$mutableblockpos.func_181079_c(j2, l2, j1)).getBlock().getMaterial().isLiquid()) {
               return true;
            }
         }
      }
