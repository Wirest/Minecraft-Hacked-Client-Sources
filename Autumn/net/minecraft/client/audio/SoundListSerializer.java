         ...
         for(int i = 0; i < jsonarray.size(); ++i) {
            ...
            if (JsonUtils.isString(jsonelement)) {
               ...
               if (jsonobject1.has("volume")) {
                  f1 = JsonUtils.getFloat(jsonobject1, "volume");
                  Validate.isTrue(f1 > 0.0F, "Invalid volume", new Object[0]);
                  soundlist$soundentry.setSoundEntryVolume(f1);
               }

               if (jsonobject1.has("pitch")) {
                  f1 = JsonUtils.getFloat(jsonobject1, "pitch");
