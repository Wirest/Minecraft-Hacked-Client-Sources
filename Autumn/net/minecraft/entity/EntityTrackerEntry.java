   ...
   public void updatePlayerList(List p_73122_1_) {
      ...
      if (this.updateCounter % this.updateFrequency == 0 || this.trackedEntity.isAirBorne || this.trackedEntity.getDataWatcher().hasObjectChanged()) {
         ...
         if (this.trackedEntity.ridingEntity == null) {
            ...
            k = MathHelper.floor_double(this.trackedEntity.posX * 32.0D);
            j1 = MathHelper.floor_double(this.trackedEntity.posY * 32.0D);
            ...
            k = MathHelper.floor_float(this.trackedEntity.rotationYaw * 256.0F / 360.0F);
            j1 = MathHelper.floor_float(this.trackedEntity.rotationPitch * 256.0F / 360.0F);
            boolean flag2 = Math.abs(k - this.encodedRotationYaw) >= 4 || Math.abs(j1 - this.encodedRotationPitch) >= 4;
            if (flag2) {
               this.sendPacketToTrackedPlayers(new S14PacketEntity.S16PacketEntityLook(this.trackedEntity.getEntityId(), (byte)k, (byte)j1, this.trackedEntity.onGround));
               this.encodedRotationYaw = k;
         ...
         k = MathHelper.floor_float(this.trackedEntity.getRotationYawHead() * 256.0F / 360.0F);
         if (Math.abs(k - this.lastHeadMotion) >= 4) {
            this.sendPacketToTrackedPlayers(new S19PacketEntityHeadLook(this.trackedEntity, (byte)k));
            this.lastHeadMotion = k;
         }
         ...
         if (this.func_180233_c(playerMP)) {
            if (!this.trackingPlayers.contains(playerMP) && (this.isPlayerWatchingThisChunk(playerMP) || this.trackedEntity.forceSpawn)) {
               ...
               if (this.trackedEntity instanceof EntityLivingBase) {
                  ...
                  Iterator var11 = entitylivingbase.getActivePotionEffects().iterator();
      ...
      if (this.trackedEntity instanceof EntityItem) {
         ...
         if (this.trackedEntity instanceof EntityFireball) {
            ...
            if (entityfireball.shootingEntity != null) {
               s0epacketspawnobject = new S0EPacketSpawnObject(this.trackedEntity, i, ((EntityFireball)this.trackedEntity).shootingEntity.getEntityId());
            } else {
               s0epacketspawnobject = new S0EPacketSpawnObject(this.trackedEntity, i, 0);
            }

            s0epacketspawnobject.setSpeedX((int)(entityfireball.accelerationX * 8000.0D));
            s0epacketspawnobject.setSpeedY((int)(entityfireball.accelerationY * 8000.0D));
            s0epacketspawnobject.setSpeedZ((int)(entityfireball.accelerationZ * 8000.0D));
            ...
            if (this.trackedEntity instanceof EntityItemFrame) {
               EntityItemFrame entityitemframe = (EntityItemFrame)this.trackedEntity;
               s0epacketspawnobject = new S0EPacketSpawnObject(this.trackedEntity, 71, entityitemframe.facingDirection.getHorizontalIndex());
               blockpos = entityitemframe.getHangingPosition();
               ...
               blockpos = entityleashknot.getHangingPosition();
               s0epacketspawnobject.setX(MathHelper.floor_float((float)(blockpos.getX() * 32)));
               s0epacketspawnobject.setY(MathHelper.floor_float((float)(blockpos.getY() * 32)));
               s0epacketspawnobject.setZ(MathHelper.floor_float((float)(blockpos.getZ() * 32)));
               return s0epacketspawnobject;
