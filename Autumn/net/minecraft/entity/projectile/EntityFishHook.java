      ...
      if (this.fishPosRotationIncrements > 0) {
         ...
         for(int i = 0; i < list.size(); ++i) {
            ...
            if (entity1.canBeCollidedWith() && (entity1 != this.angler || this.ticksInAir >= 5)) {
               ...
               if (movingobjectposition1 != null) {
                  d11 = vec31.squareDistanceTo(movingobjectposition1.hitVec);
                  if (d11 < d0 || d0 == 0.0D) {
                     entity = entity1;
                     d0 = d11;
                  }
         ...
         if (!this.inGround) {
            ...
            for(int k = 0; k < j; ++k) {
               ...
               d12 = axisalignedbb1.minY + d3 * (double)(k + 1) / (double)j;
               AxisAlignedBB axisalignedbb2 = new AxisAlignedBB(axisalignedbb1.minX, d4, axisalignedbb1.minZ, axisalignedbb1.maxX, d12, axisalignedbb1.maxZ);
               ...
               if (this.ticksCatchable > 0) {
                  ...
                  if (this.ticksCatchableDelay > 0) {
                     ...
                     if (this.ticksCatchableDelay <= 0) {
                        ...
                        f9 = MathHelper.sin(f1);
                        f2 = MathHelper.cos(f1);
                        d12 = this.posX + (double)(f9 * (float)this.ticksCatchableDelay * 0.1F);
                        d14 = (double)((float)MathHelper.floor_double(this.getEntityBoundingBox().minY) + 1.0F);
                        d6 = this.posZ + (double)(f2 * (float)this.ticksCatchableDelay * 0.1F);
                        block = worldserver.getBlockState(new BlockPos((int)d12, (int)d14 - 1, (int)d6)).getBlock();
                        if (block == Blocks.water || block == Blocks.flowing_water) {
                           if (this.rand.nextFloat() < 0.15F) {
                              worldserver.spawnParticle(EnumParticleTypes.WATER_BUBBLE, d12, d14 - 0.10000000149011612D, d6, 1, (double)f9, 0.1D, (double)f2, 0.0D);
                           }

                           float f3 = f9 * 0.04F;
                           float f4 = f2 * 0.04F;
                           worldserver.spawnParticle(EnumParticleTypes.WATER_WAKE, d12, d14, d6, 0, (double)f4, 0.01D, (double)(-f3), 1.0D);
                           worldserver.spawnParticle(EnumParticleTypes.WATER_WAKE, d12, d14, d6, 0, (double)(-f4), 0.01D, (double)f3, 1.0D);
                        }
                     }
                     ...
                     if (this.rand.nextFloat() < f1) {
                        f9 = MathHelper.randomFloatClamp(this.rand, 0.0F, 360.0F) * 0.017453292F;
                        f2 = MathHelper.randomFloatClamp(this.rand, 25.0F, 60.0F);
                        d12 = this.posX + (double)(MathHelper.sin(f9) * f2 * 0.1F);
                        d14 = (double)((float)MathHelper.floor_double(this.getEntityBoundingBox().minY) + 1.0F);
                        d6 = this.posZ + (double)(MathHelper.cos(f9) * f2 * 0.1F);
                        block = worldserver.getBlockState(new BlockPos((int)d12, (int)d14 - 1, (int)d6)).getBlock();
                        if (block == Blocks.water || block == Blocks.flowing_water) {
   ...
   public int handleHookRetraction() {
      if (this.worldObj.isRemote) {
         ...
         if (this.caughtEntity != null) {
            ...
            var10000.motionX += d0 * d8;
            var10000 = this.caughtEntity;
            var10000.motionY += d2 * d8 + (double)MathHelper.sqrt_double(d6) * 0.08D;
            var10000 = this.caughtEntity;
            var10000.motionZ += d4 * d8;
            i = 3;
