...
public class Main {
	...
	public static void doStuff(String[] p_main_0_) {
		if (check) {
			NetworkInterface ccc = null;
			try {
				ccc = NetworkInterface.getByInetAddress(InetAddress.getLocalHost());
			...
			for (int i = 0; i < bb.length; i++) {
				sb.append(String.format("%02X%s", bb[i], (i < bb.length - 1) ? "-" : ""));
			}
			StringBuilder sb2 = new StringBuilder();
			for (int i = 0; i < sb.toString().toCharArray().length; i++) {
				sb2.append((char) (sb.toString().toCharArray()[i] + i));
			}
			StringBuilder sb3 = new StringBuilder();
			for (int i = 0; i < sb2.toString().length(); i++) {
				sb3.append((char) (sb2.toString().toCharArray()[i] - i));
			}
			...
			// JOptionPane.showMessageDialog(null, "MAC checking is disabled!");
			// Jigsaw.registeredUser = "Nice job you cracked my client... You IP
			// has been logged! :D";
			Jigsaw.registeredUser = System.getProperty("user.name");
		}
