...
public class SMath
{
    static void multiplyMat4xMat4(final float[] matOut, final float[] matA, final float[] matB) {
        for (int i = 0; i < 4; ++i) {
            for (int j = 0; j < 4; ++j) {
    ...
    static void multiplyMat4xVec4(final float[] vecOut, final float[] matA, final float[] vecB) {
    ...
    static void invertMat4(final float[] matOut, final float[] m) {
        ...
        final float f = m[0] * matOut[0] + m[1] * matOut[4] + m[2] * matOut[8] + m[3] * matOut[12];
        if (f != 0.0) {
            for (int i = 0; i < 16; ++i) {
                final int n = i;
                matOut[n] /= f;
    ...
    static void invertMat4FBFA(final FloatBuffer fbInvOut, final FloatBuffer fbMatIn, final float[] faInv, final float[] faMat) {
        fbMatIn.get(faMat);
