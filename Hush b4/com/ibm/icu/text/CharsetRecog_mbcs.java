...
abstract class CharsetRecog_mbcs extends CharsetRecognizer
{
    ...
    int match(final CharsetDetector det, final int[] commonChars) {
        ...
        int confidence = 0;
        final iteratedChar iter = new iteratedChar();
        iter.reset();
        while (this.nextChar(iter, det)) {
            ...
            if (iter.error) {
                ...
                final long cv = (long)iter.charValue & 0xFFFFFFFFL;
                if (cv <= 255L) {
                    ++singleByteCharCount;
                }
                else {
        ...
        if (doubleByteCharCount <= 10 && badCharCount == 0) {
            ...
            if (commonChars != null) {
                final double maxVal = Math.log(doubleByteCharCount / 4.0f);
                final double scaleFactor = 90.0 / maxVal;
    ...
    static class iteratedChar
    {
        ...
        int nextByte(final CharsetDetector det) {
            ...
            final int byteValue = det.fRawInput[this.nextIndex++] & 0xFF;
            return byteValue;
        }
    }
    ...
    static class CharsetRecog_sjis extends CharsetRecog_mbcs
    {
        ...
        boolean nextChar(final iteratedChar it, final CharsetDetector det) {
            ...
            it.error = false;
            final int nextByte = it.nextByte(det);
            ...
            final int secondByte = it.nextByte(det);
            if (secondByte < 0) {
                return false;
            }
        ...
        CharsetMatch match(final CharsetDetector det) {
            final int confidence = this.match(det, CharsetRecog_sjis.commonChars);
    ...
    static class CharsetRecog_big5 extends CharsetRecog_mbcs
    {
        ...
        @Override
        CharsetMatch match(final CharsetDetector det) {
            final int confidence = this.match(det, CharsetRecog_big5.commonChars);
            return (confidence == 0) ? null : new CharsetMatch(det, this, confidence);
        }
    ...
    abstract static class CharsetRecog_euc extends CharsetRecog_mbcs
    {
        ...
        static class CharsetRecog_euc_jp extends CharsetRecog_euc
        {
            ...
            @Override
            CharsetMatch match(final CharsetDetector det) {
                final int confidence = this.match(det, CharsetRecog_euc_jp.commonChars);
                return (confidence == 0) ? null : new CharsetMatch(det, this, confidence);
            }
        ...
        static class CharsetRecog_euc_kr extends CharsetRecog_euc
        {
            ...
            @Override
            CharsetMatch match(final CharsetDetector det) {
                final int confidence = this.match(det, CharsetRecog_euc_kr.commonChars);
                return (confidence == 0) ? null : new CharsetMatch(det, this, confidence);
            }
    ...
    static class CharsetRecog_gb_18030 extends CharsetRecog_mbcs
    {
        ...
        boolean nextChar(final iteratedChar it, final CharsetDetector det) {
            ...
            if (firstByte < 0) {
                ...
                secondByte = it.nextByte(det);
                it.charValue = (it.charValue << 8 | secondByte);
                if (firstByte >= 129 && firstByte <= 254 && (secondByte < 64 || secondByte > 126)) {
                    if (secondByte < 80 || secondByte > 254) {
        ...
        @Override
        CharsetMatch match(final CharsetDetector det) {
            final int confidence = this.match(det, CharsetRecog_gb_18030.commonChars);
            return (confidence == 0) ? null : new CharsetMatch(det, this, confidence);
        }
