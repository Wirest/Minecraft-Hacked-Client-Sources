...
public abstract class GuiContainer extends GuiScreen
{
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks)
    {
		...
		blurring = AnimationUtil.slide(blurring, 0, 2, .05, true);
		Client.BLUR_UTIL.blur((int) blurring);
    	scaling = animUtil.easeOut(0, 0, 1, .5);
    	scaling = MathHelper.clamp_double(scaling, 0, 1);
    	Client.RENDER2D.gradient(0, 0, this.width, this.height, Client.INSTANCE.getClientColor().setAlpha((int) (200 * scaling)), new Color(255, 255, 255, (int) (50 * scaling)));
        int i = this.guiLeft;
        int j = this.guiTop;
        ...
        GL11.glPushMatrix();
        Client.RENDER2D.translate(width / 2, height / 2);
        Client.RENDER2D.scale(scaling, scaling);
        Client.RENDER2D.translate(-width / 2, -height / 2);
...
//        System.out.println(slot.slotNumber);
    ...
    protected void actionPerformed(GuiButton button) throws IOException {
		if (button.id == 100) {
			...
			Client.INSTANCE.getModuleManager().getModuleByClass(AutoArmor.class).setEnabled(false);
		}
		...
		if (button.id == 102) {
			mc.thePlayer.closeScreen();
			Client.INSTANCE.getModuleManager().getModuleByClass(InventoryCleaner.class).setEnabled(false);
		}
		
		if (button.id == 103) {
			mc.thePlayer.closeScreen();
			Client.INSTANCE.getModuleManager().getModuleByClass(Aura.class).setEnabled(false);
		}
