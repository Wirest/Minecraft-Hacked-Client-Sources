...
public class GuiNewChat extends Gui
{
    ...
    public void drawChat(int p_146230_1_)
    {
        if (this.mc.gameSettings.chatVisibility != EntityPlayer.EnumChatVisibility.HIDDEN)
        {
            ...
            if (k > 0)
            {
                ...
                {
                    final ChatLine chatline = (ChatLine)this.field_146253_i.get(i1 + this.scrollPos);
    ...
    public void printChatMessageWithOptionalDeletion(final IChatComponent p_146234_1_, final int p_146234_2_) {
        ...
        final String text = p_146234_1_.getUnformattedText().toLowerCase();
        if (Client.INSTANCE.getModuleManager().getModuleByClass(AutoPlay.class).isEnabled()) {
            if (text.contains("play again") || text.contains("volver a jugar")) {
                this.mc.thePlayer.sendChatMessage("/playagain now");
            }
            else if (text.contains("want to play again?")) {
                this.mc.thePlayer.sendChatMessage("/play solo_normal");
            }
        }
        if (Client.INSTANCE.getModuleManager().getModuleByClass(AutoRegister.class).isEnabled()) {
            if (text.contains("/register")) {
                this.mc.thePlayer.sendChatMessage("/register vulture vulture");
                Client.PRINTER.printMessage(String.format("Registered with password '%s%s§f'", Client.INSTANCE.getClientColorCode(), "vulture"));
            }
            else if (text.contains("/login")) {
                this.mc.thePlayer.sendChatMessage("/login vulture");
                Client.PRINTER.printMessage(String.format("Logged in with password '%s%s§f'", Client.INSTANCE.getClientColorCode(), "vulture"));
            }
        }
    ...
    public IChatComponent getChatComponent(int p_146236_1_, int p_146236_2_)
    {
        ...
        {
        	final TTFFontRenderer font = Client.INSTANCE.getFontManager().getDefaultFont();
...
//                if (j <= MathHelper.floor_float((float)this.getChatWidth() / this.getChatScale()) && k < this.mc.fontRendererObj.FONT_HEIGHT * l + l)
                if (j <= MathHelper.floor_float((float)this.getChatWidth() / this.getChatScale()) && k < font.getHeight(chatLines.get(0).getChatComponent().getUnformattedText()) * l + l)
                {
...
//                                j1 += this.mc.fontRendererObj.getStringWidth(GuiUtilRenderComponents.func_178909_a(((ChatComponentText)ichatcomponent).getChatComponentText_TextValue(), false));
                            	j1 += font.getWidth(GuiUtilRenderComponents.func_178909_a(((ChatComponentText)ichatcomponent).getChatComponentText_TextValue(), false));
