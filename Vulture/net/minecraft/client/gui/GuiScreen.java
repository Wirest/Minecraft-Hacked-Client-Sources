...
public abstract class GuiScreen extends Gui implements GuiYesNoCallback
{
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks)
    {
        ...
        deltaTicks += Client.DELTA_UTIL.deltaTime * .01F;
    }
...
//                int j = this.fontRendererObj.getStringWidth(s);
            	int j = (int) Client.INSTANCE.getFontManager().getDefaultFont().getWidth(s);
            ...
            int l = -267386864;
            Client.RENDER2D.rect(l1 - 3, i2 - 2, i + 4, k + 4, new Color(0, 0, 0, 200));
            int i1 = Client.INSTANCE.getClientColor().getRGB();
            int j1 = Client.INSTANCE.getClientColor().getRGB();
...
//                this.fontRendererObj.drawStringWithShadow(s1, (float)l1 - 1, (float)i2, -1);
                Client.INSTANCE.getFontManager().getDefaultFont().drawStringWithShadow(s1, l1 - 1, i2, Color.white);
     ...
     */
    public void drawDefaultBackground(int mouseX, int mouseY)
    {
        this.drawWorldBackground(mouseX, mouseY);
    }

    public void drawWorldBackground(int mouseX, int mouseY)
    {
        if (this.mc.theWorld != null)
        {
        	Client.RENDER2D.gradient(0, 0, this.width, this.height, Client.INSTANCE.getClientColor().setAlpha(200), new Color(255, 255, 255, 50));
        }
        else
        {
    ...
    {
		if (mouse == null || animUtil == null) {
			if (Client.INSTANCE.getBackgroundPath().contains("client/designs")) {
				Client.RENDER2D.image(new ResourceLocation(Client.INSTANCE.getBackgroundPath()), 0, 0, width, height);
			} else {
				if (Client.INSTANCE.getCustomBackground() != null) {
					Client.INSTANCE.getCustomBackground().draw(0, 0, width, height);
				}
			}
			return;
		}
		mouse.set(mouseX, mouseY);
		Vec diff = mouse.sub(center);
		double tX = (diff.x - width / 2) / width * -40;
		double tY = (diff.y - height / 2) / height * -40;
		double x = tX;
		double y = tY;
		if (Client.INSTANCE.getBackgroundPath().contains("client/designs")) {
			Client.RENDER2D.image(new ResourceLocation(Client.INSTANCE.getBackgroundPath()), -x / 2, -y / 2, width + 40, height + 40);
		} else {
			if (Client.INSTANCE.getCustomBackground() != null) {
				Client.INSTANCE.getCustomBackground().draw(-x / 2, -y / 2, width + 40, height + 40);
			}
		}
    }
