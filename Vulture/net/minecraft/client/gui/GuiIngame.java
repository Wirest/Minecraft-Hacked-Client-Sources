...
//    	EventManager.call(new RenderEvent(RenderState.TWOD, partialTicks));
        ...
        {
        	if (!(hud.isEnabled() && hud.getValueByName("HUDCrosshair").getBooleanValue())) {
        ...
        {
        	if (!(hud.isEnabled() && hud.getValueByName("HUDBetterHUD").getBooleanValue())) {
        ...
        if (this.recordPlayingUpFor > 0)
        {
            ...
            if (k1 > 8)
            {
                ...
                this.getFontRenderer().drawString(this.recordPlaying, -this.getFontRenderer().getStringWidth(this.recordPlaying) / 2, -4 - (hud.isEnabled() && hud.getValueByName("HUDBetterHUD").getBooleanValue() ? 50 : 0), i1 + (k1 << 24 & -16777216));
                GlStateManager.disableBlend();
        ...
        if (Client.INSTANCE.getModuleManager().getModuleByClass(NoHurtCam.class).isEnabled()) {
			if (mc.thePlayer.hurtTime > 0) {
				drawGradientRect(0, 0, scaledresolution.getScaledWidth(), scaledresolution.getScaledHeight(), new Color(255, 0, 0, mc.thePlayer.hurtTime * 15).getRGB(), 0);
			}
        }
    }
    ...
    protected void renderTooltip(ScaledResolution sr, float partialTicks)
    {
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer)
        {
            ...
            if (Client.INSTANCE.getModuleManager().getModuleByClass(HUD.class).getValueByName("HUDKöftespieß").getBooleanValue()) {
            	Client.RENDER2D.image(new ResourceLocation("client/textures/koefte.png"), i - width / 2, sr.getScaledHeight() - 20, width, 20);
            } else {
            	if (Client.INSTANCE.getModuleManager().getModuleByClass(HUD.class).isEnabled() && Client.INSTANCE.getModuleManager().getModuleByClass(HUD.class).getValueByName("HUDInfo").getBooleanValue()) {
            		Client.BLUR_UTIL.blur(0, sr.getScaledHeight() - 20, sr.getScaledWidth(), 20, 5);
            		Client.RENDER2D.rect(0, sr.getScaledHeight() - 20, sr.getScaledWidth(), 20, new Color(20, 20, 20, 200));
            	} else {
            		Client.BLUR_UTIL.blur(i - width / 2, sr.getScaledHeight() - 20, width, 20, 5);
            		Client.RENDER2D.rect(i - width / 2, sr.getScaledHeight() - 20, width, 20, new Color(20, 20, 20, 200));
            	}
            }
            
			if (Client.INSTANCE.getModuleManager().getModuleByClass(HUD.class).getValueByName("HUDInfo").getBooleanValue()) {
				drawInfo(sr);
			}
			if (!Client.INSTANCE.getModuleManager().getModuleByClass(HUD.class).getValueByName("HUDInfo").getBooleanValue() || !Client.INSTANCE.getModuleManager().getModuleByClass(HUD.class).isEnabled()) {
				animUtil.getProgression(0).reset();
			}
            
			Client.RENDER2D.rect(i - width / 2 + entityplayer.inventory.currentItem * 20, sr.getScaledHeight() - 20.5, 20, 20.5, new Color(200, 200, 200, 200));
	...
	void drawInfo(ScaledResolution sr) {
		final String coodinates = String.format("X: %s%s§f | Y: %s%s§f | Z: %s%s§f", "§8", (int)mc.thePlayer.posX, "§8", (int)mc.thePlayer.posY, "§8", (int)mc.thePlayer.posZ);
		final String ping = String.format("Ping: %s%s§f", "§8", mc.isSingleplayer() ? "0" : mc.getCurrentServerData().pingToServer);
		final String fps = String.format("FPS: %s%s", "§8", mc.getDebugFPS());
		
		final String server = String.format("Server: %s%s§f", "§8", mc.isSingleplayer() ? "Singleplayer" : mc.getCurrentServerData().serverIP);
		final String username = String.format("User: %s%s§f", "§8", mc.thePlayer.getName());

		final double offset = 1;

		// Left bar
		
		final double coordsWidth = Client.INSTANCE.getFontManager().getDefaultFont().getWidth(coodinates);
		final double coordsHeight = Client.INSTANCE.getFontManager().getDefaultFont().getHeight(coodinates);
		
		final double pingWidth = Client.INSTANCE.getFontManager().getDefaultFont().getWidth(ping);
		final double pingHeight = Client.INSTANCE.getFontManager().getDefaultFont().getHeight(ping);
		
		final double fpsWidth = Client.INSTANCE.getFontManager().getDefaultFont().getWidth(fps);
		final double fpsHeight = Client.INSTANCE.getFontManager().getDefaultFont().getHeight(fps);
		
		final double leftBarHeight = (coordsHeight + pingHeight + offset);
		
		GlStateManager.enableAlpha();
		
		Client.INSTANCE.getFontManager().getDefaultFont().drawStringWithShadow(coodinates, offset, (sr.getScaledHeight() - coordsHeight - offset), Color.white);
		Client.INSTANCE.getFontManager().getDefaultFont().drawStringWithShadow(ping, offset, sr.getScaledHeight() - leftBarHeight, Color.white);
		Client.INSTANCE.getFontManager().getDefaultFont().drawStringWithShadow(fps, pingWidth + offset * 2, sr.getScaledHeight() - leftBarHeight, Color.white);
		
		// Right bar
		final double serverWidth = Client.INSTANCE.getFontManager().getDefaultFont().getWidth(server);
		final double serverHeight = Client.INSTANCE.getFontManager().getDefaultFont().getHeight(server);

		final double usernameWidth = Client.INSTANCE.getFontManager().getDefaultFont().getWidth(username);
		final double usernameHeight = Client.INSTANCE.getFontManager().getDefaultFont().getHeight(username);
		
		final double rightBarHeight = (serverHeight + usernameHeight + offset);
		
		Client.INSTANCE.getFontManager().getDefaultFont().drawStringWithShadow(server, sr.getScaledWidth() - serverWidth, (sr.getScaledHeight() - serverHeight - offset), Color.white);
		Client.INSTANCE.getFontManager().getDefaultFont().drawStringWithShadow(username, sr.getScaledWidth() - usernameWidth, sr.getScaledHeight() - rightBarHeight, Color.white);
    ...
    private void renderScoreboard(ScoreObjective p_180475_1_, ScaledResolution p_180475_2_)
    {
        ...
        final ArrayList arraylist = Lists.newArrayList(Iterables.filter(collection, new Predicate()
        {
...
//        int i = this.getFontRenderer().getStringWidth(p_180475_1_.getDisplayName());
        int i = (int) Client.INSTANCE.getFontManager().getDefaultFont().getWidth(p_180475_1_.getDisplayName());

        for (final Object score : arraylist1)
        {
            ...
            i = (int) Math.max(i, Client.INSTANCE.getFontManager().getDefaultFont().getWidth(s));
        }
        
        double clientArrayListYOffset = 0;
        ...
        if (Client.INSTANCE.getModuleManager().getModuleByClass(HUD.class).isEnabled()) {
			for (final Module module : Client.INSTANCE.getModuleManager().getModules()) {
				...
				double moduleWidth = Client.INSTANCE.getFontManager().getDefaultFont().getWidth(module.getName() + module.getSettingInfo()) + 4;
				double moduleHeight = Client.INSTANCE.getFontManager().getDefaultFont().getHeight(module.getName() + module.getSettingInfo()) + 2;
				if (module.isEnabled() || (module.getAnimX() != 0 && module.getAnimX() != moduleWidth)) {
					clientArrayListYOffset += moduleHeight - module.getAnimY();
				}
			}
        }
        
        int j1 = arraylist1.size() * this.getFontRenderer().FONT_HEIGHT + 10;
        double k1 = p_180475_2_.getScaledHeight() / 2 + j1 / 3 + (clientArrayListYOffset > p_180475_2_.getScaledHeight() / 2 - j1 - 20 ? clientArrayListYOffset - p_180475_2_.getScaledHeight() / 2 + j1 - 20 : 0);
        byte b0 = 3;
        int j = p_180475_2_.getScaledWidth() - i - b0;
        int k = 0;
        
        if (Client.INSTANCE.getModuleManager().getModuleByClass(NoScoreboard.class).isEnabled()) return;
        
        for (final Object score1 : arraylist1)
        {
            ++k;
...
//            double l = k1 - k * this.getFontRenderer().FONT_HEIGHT;
            double l = k1 - k * Client.INSTANCE.getFontManager().getDefaultFont().getHeight(s2);
            int i1 = p_180475_2_.getScaledWidth() - b0 + 2;
//            drawRect(j - 2, l, i1, l + this.getFontRenderer().FONT_HEIGHT, 1342177280);
//            drawRect(j - 2, l, i1, l + Client.INSTANCE.getFontManager().getDefaultFont().getHeight(s2), 1342177280);
            Client.RENDER2D.rect(j - 2, l, i1, Client.INSTANCE.getFontManager().getDefaultFont().getHeight(s2), color);
//            this.getFontRenderer().drawString(s1, j, l, 553648127);
            Client.INSTANCE.getFontManager().getDefaultFont().drawStringWithShadow(s1, j, l, Color.white);
            Client.INSTANCE.getFontManager().getDefaultFont().drawStringWithShadow(s2, i1 - Client.INSTANCE.getFontManager().getDefaultFont().getWidth(s2), l, Color.white);
...
//                drawRect(j - 2, l - Client.INSTANCE.getFontManager().getDefaultFont().getHeight(s3) - 1, i1, l - 1, 1610612736);
//                drawRect(j - 2, l - 1, i1, l, 1342177280);
                
                Client.RENDER2D.rect(j - 2, l - Client.INSTANCE.getFontManager().getDefaultFont().getHeight(s3) - 1, i1, Client.INSTANCE.getFontManager().getDefaultFont().getHeight(s3) + 1, color);
...
//                this.getFontRenderer().drawString(s3, j + i / 2 - this.getFontRenderer().getStringWidth(s3) / 2, l - this.getFontRenderer().FONT_HEIGHT, 553648127);
                Client.INSTANCE.getFontManager().getDefaultFont().drawStringWithShadow(s3, j + i / 2 - Client.INSTANCE.getFontManager().getDefaultFont().getWidth(s3) / 2, l - Client.INSTANCE.getFontManager().getDefaultFont().getHeight(s3), Color.white);
            }
        }
    }
    ...
    private void renderHotbarItem(int index, double xPos, double yPos, float partialTicks, EntityPlayer p_175184_5_)
    {
