...
public class ItemRenderer
{
    ...
    private void transformFirstPersonItem(float equipProgress, float swingProgress)
    {
    	...
    	if (changer.isEnabled() && changer.getValueByName("RenderChangerHigher held item").getBooleanValue()) {
    ...
    private void func_178103_d()
    {
    	final RenderChanger changer = (RenderChanger) Client.INSTANCE.getModuleManager().getModuleByClass(RenderChanger.class);
    	if (changer.isEnabled() && changer.getValueByName("RenderChangerHigher held item").getBooleanValue()) {
    		GlStateManager.translate(-.8F, 0.2F, -.2F);
    	} else {
    ...
    public void renderItemInFirstPerson(float partialTicks)
    {
        ...
        if (this.itemToRender != null)
        {
        	...
        	final Swing swing = (Swing) Client.INSTANCE.getModuleManager().getModuleByClass(Swing.class);
        	final RenderChanger changer = (RenderChanger) Client.INSTANCE.getModuleManager().getModuleByClass(RenderChanger.class);
            ...
            else if (entityplayersp.getItemInUseCount() > 0 || (itemToRender.getItem() instanceof ItemSword && aura.isEnabled() && false || ((mc.gameSettings.keyBindUseItem.pressed || false) && changer.isEnabled() && changer.getValueByName("RenderChangerItem block").getBooleanValue()) && !(itemToRender.getItem() instanceof ItemBlock) && !(itemToRender.getItem() instanceof ItemFood)))
            {
                EnumAction enumaction = false ? EnumAction.BLOCK : changer.getValueByName("RenderChangerItem block").getBooleanValue() && changer.isEnabled() ? itemToRender.getItemUseAction() == EnumAction.NONE ? EnumAction.BLOCK : itemToRender.getItemUseAction() : itemToRender.getItemUseAction();
                ...
                switch (ItemRenderer.ItemRenderer$1.field_178094_a[enumaction.ordinal()])
                {
                    ...
                    case 4:
                    	if (swing.getValueByName("SwingBlockModes").isCombo("Stab")) {
                    		this.transformFirstPersonItem(f, f1 - 1);
                    	} else {
                    		this.transformFirstPersonItem(f / 2.0F, f1 * 0.8F);
                    	}
                        this.func_178103_d();
                        if (entityplayersp.isSwingInProgress)
                        	GlStateManager.translate(-.4, .2, -.5);
                        break;
            ...
            {
            	if (!swing.getValueByName("SwingModes").getComboValue().equalsIgnoreCase("Lowswing")) {
