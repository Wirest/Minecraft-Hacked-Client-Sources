...
public class EntityRenderer implements IResourceManagerReloadListener {
	...
	private static final ResourceLocation[] shaderResourceLocations = new ResourceLocation[] { new ResourceLocation("shaders/post/notch.json"), new ResourceLocation("shaders/post/fxaa.json"), new ResourceLocation("shaders/post/art.json"), new ResourceLocation("shaders/post/bumpy.json"), new ResourceLocation("shaders/post/blobs2.json"), new ResourceLocation("shaders/post/pencil.json"), new ResourceLocation("shaders/post/color_convolve.json"), new ResourceLocation("shaders/post/deconverge.json"), new ResourceLocation("shaders/post/flip.json"), new ResourceLocation("shaders/post/invert.json"), new ResourceLocation("shaders/post/ntsc.json"), new ResourceLocation("shaders/post/outline.json"), new ResourceLocation("shaders/post/phosphor.json"), new ResourceLocation("shaders/post/scan_pincushion.json"), new ResourceLocation("shaders/post/sobel.json"), new ResourceLocation("shaders/post/bits.json"), new ResourceLocation("shaders/post/desaturate.json"), new ResourceLocation("shaders/post/green.json"), new ResourceLocation("shaders/post/blur.json"), new ResourceLocation("shaders/post/blur2.json"), new ResourceLocation("shaders/post/wobble.json"), new ResourceLocation("shaders/post/blobs.json"), new ResourceLocation("shaders/post/antialias.json"), new ResourceLocation("shaders/post/creeper.json"), new ResourceLocation("shaders/post/spider.json") };
	...
	public void updateRenderer() {
		...
		final double d1 = entity.posY + (double) entity.getEyeHeight();
		final double d2 = entity.posZ;
		final float f3 = this.mc.theWorld.getLightBrightness(new BlockPos(d0, d1, d2));
		float f4 = (float) this.mc.gameSettings.renderDistanceChunks / 16.0F;
		f4 = MathHelper.clamp_float(f4, 0.0F, 1.0F);
		final float f2 = f3 * (1.0F - f4) + f4;
	...
	public void getMouseOver(final float partialTicks) {
		...
		if (entity != null && this.mc.theWorld != null) {
			...
			double d0 = (reach.isEnabled() ? reach.getMaxRange() : this.mc.playerController.getBlockReachDistance());
			this.mc.objectMouseOver = entity.rayTrace(reach.isEnabled() ? reach.getInteractRange() : d0, partialTicks);
			double d1 = d0;
			...
			if (this.pointedEntity != null && flag && vec3.distanceTo(vec33) > (reach.isEnabled() ? reach.getAttackRange() : 3.0D)) {
				this.pointedEntity = null;
			...
			if (this.mc.gameSettings.debugCamEnable) {
				...
				for (int i = 0; i < 8; ++i) {
					...
					if (movingobjectposition != null) {
						...
						if (!Client.INSTANCE.getModuleManager().getModuleByClass(CamClip.class).isEnabled()) {
							if (d7 < d3) {
								d3 = d7;
							}
						}
	...
	public void setupCameraTransform(final float partialTicks, final int pass) {
		...
		if (this.mc.gameSettings.viewBobbing) {
			if (Client.INSTANCE.getModuleManager().getModuleByClass(Tracers.class).isEnabled())
				GL11.glPushMatrix();
		...
		if (this.lightmapUpdateNeeded) {
			...
			if (worldclient != null) {
				...
				for (int i = 0; i < 256; ++i) {
					...
					final float f3 = worldclient.provider.getLightBrightnessTable()[i % 16] * (this.torchFlickerX * 0.1F + 1.5F);
	...
	public void func_181560_a(final float p_181560_1_, final long p_181560_2_) {
		...
		if (!this.mc.skipRenderWorld) {
			...
			final int l1 = this.mc.gameSettings.limitFramerate;

			if (this.mc.theWorld != null) {
				this.mc.mcProfiler.startSection("level");
				int i = Math.min(Minecraft.getDebugFPS(), l1);
				i = Math.max(i, 60);
				final long j = System.nanoTime() - p_181560_2_;
				final long k = Math.max((long) (1000000000 / i / 4) - j, 0L);
	...
	public void renderWorld(final float partialTicks, final long finishTimeNano) {
		final RearView view = (RearView) Client.INSTANCE.getModuleManager().getModuleByClass(RearView.class);
		...
		if (this.mc.gameSettings.anaglyph) {
			...
			if (view.isEnabled()) {
				renderWorldPass2(0, partialTicks, finishTimeNano);
			}
			anaglyphField = 1;
			GlStateManager.colorMask(true, false, false, false);
			this.renderWorldPass(1, partialTicks, finishTimeNano);
			if (view.isEnabled()) {
				renderWorldPass2(1, partialTicks, finishTimeNano);
			}
			GlStateManager.colorMask(true, true, true, false);
		} else {
			this.renderWorldPass(2, partialTicks, finishTimeNano);
			if (view.isEnabled()) {
				renderWorldPass2(3, partialTicks, finishTimeNano);
			}
		}
	...
	private void renderWorldPass(final int pass, final float partialTicks, final long finishTimeNano) {
		...
		EventManager.call(new RenderEvent(RenderState.THREED, partialTicks));
	...
	private void addRainParticles() {
		...
		if (f != 0.0F && Config.isRainSplash()) {
			...
			for (int k = 0; k < j; ++k) {
				final BlockPos blockpos1 = worldclient.getPrecipitationHeight(blockpos.add(this.random.nextInt(b0) - this.random.nextInt(b0), 0, this.random.nextInt(b0) - this.random.nextInt(b0)));
				final BiomeGenBase biomegenbase = worldclient.getBiomeGenForCoords(blockpos1);
		...
		if (f5 > 0.0F) {
			...
			for (int i1 = k - b0; i1 <= k + b0; ++i1) {
				for (int j1 = i - b0; j1 <= i + b0; ++j1) {
					...
					final double d3 = this.rainXCoords[k1] * 0.5D;
					final double d4 = this.rainYCoords[k1] * 0.5D;
					...
					if (biomegenbase.canSpawnLightningBolt() || biomegenbase.getEnableSnow()) {
						...
						if (i2 != j2) {
							...
							if (worldclient.getWorldChunkManager().getTemperatureAtHeight(f1, l1) >= 0.15F) {
								...
								final double d5 = ((this.rendererUpdateCount + j1 * j1 * 3121 + j1 * 45238971 + i1 * i1 * 418711 + i1 * 13761 & 31) + partialTicks) / 32.0D * (3.0D + this.random.nextDouble());
								final double d6 = (j1 + 0.5F) - entity.posX;
								final double d7 = (i1 + 0.5F) - entity.posZ;
								final float f2 = MathHelper.sqrt_double(d6 * d6 + d7 * d7) / (float) b0;
								...
								final double d9 = this.random.nextDouble() +  f * 0.01D *  ( this.random.nextGaussian());
								final double d10 = this.random.nextDouble() +  (f *  this.random.nextGaussian()) * 0.001D;
								final double d11 =  ( j1 + 0.5F) - entity.posX;
								final double d12 =  ( i1 + 0.5F) - entity.posZ;
								final float f6 = MathHelper.sqrt_double(d11 * d11 + d12 * d12) /  b0;
								final float f4 = ((1.0F - f6 * f6) * 0.3F + 0.5F) * f5;
								blockpos$mutableblockpos.func_181079_c(j1, k2, i1);
								final int k3 = (worldclient.getCombinedLight(blockpos$mutableblockpos, 0) * 3 + 15728880) / 4;
								final int l3 = k3 >> 16 & 65535;
								final int i4 = k3 & 65535;
	...
	private void updateFogColor(final float partialTicks) {
		...
		if (this.mc.gameSettings.renderDistanceChunks >= 4) {
			double d0 = -1.0D;
			final Vec3 vec32 = MathHelper.sin(worldclient.getCelestialAngleRadians(partialTicks)) > 0.0F ? new Vec3(d0, 0.0D, 0.0D) : new Vec3(1.0D, 0.0D, 0.0D);
		...
		final float f10 = worldclient.getRainStrength(partialTicks);

		if (f10 > 0.0F) {
			final float f5 = 1.0F - f10 * 0.5F;
			final float f12 = 1.0F - f10 * 0.4F;
			this.fogColorRed *= f5;
			this.fogColorGreen *= f5;
			this.fogColorBlue *= f12;
		}

		final float f11 = worldclient.getThunderStrength(partialTicks);

		if (f11 > 0.0F) {
			final float f13 = 1.0F - f11 * 0.5F;
		...
		if (this.cloudFog) {
			final Vec3 vec33 = worldclient.getCloudColour(partialTicks);
			...
			final Vec3 vec34 = CustomColors.getUnderwaterColor(this.mc.theWorld, this.mc.getRenderViewEntity().posX, this.mc.getRenderViewEntity().posY + 1.0D, this.mc.getRenderViewEntity().posZ);
		...
		final float f9 = this.fogColor2 + (this.fogColor1 - this.fogColor2) * partialTicks;
		this.fogColorRed *= f9;
		...
		if (Reflector.EntityViewRenderEvent_FogColors_Constructor.exists()) {
			final Object object = Reflector.newInstance(Reflector.EntityViewRenderEvent_FogColors_Constructor, new Object[] { this, entity, block, Float.valueOf(partialTicks), Float.valueOf(this.fogColorRed), Float.valueOf(this.fogColorGreen), Float.valueOf(this.fogColorBlue) });
			Reflector.postForgeBusEvent(object);
	...
	private void frameInit() {
		...
		if (world != null) {
			if (Config.getNewRelease() != null) {
				...
				final ChatComponentText chatcomponenttext = new ChatComponentText(I18n.format("of.message.newVersion", new Object[] { s1 }));
			...
			if (Config.isNotify64BitJava()) {
				...
				final ChatComponentText chatcomponenttext1 = new ChatComponentText(I18n.format("of.message.java64Bit", new Object[0]));
		...
		if (this.mc.theWorld != null) {
			...
			if (i > this.lastErrorCheckTimeMs + 10000L) {
				...
				if (j != 0) {
					...
					final ChatComponentText chatcomponenttext = new ChatComponentText(I18n.format("of.message.openglError", new Object[] { Integer.valueOf(j), s }));
					this.mc.ingameGUI.getChatGUI().printChatMessage(chatcomponenttext);
				}
			}
		}
