...
public class EnchantmentHelper
{
    ...
    public static int getEfficiencyModifier(EntityLivingBase player)
    {
    	...
    	final AutoTool autoTool = (AutoTool) Client.INSTANCE.getModuleManager().getModuleByClass(AutoTool.class);
    	if (autoTool.isEnabled() && autoTool.getValueByName("AutoToolSilent").getBooleanValue() && Minecraft.getMinecraft().objectMouseOver.typeOfHit == MovingObjectType.BLOCK) {
    		if (player instanceof EntityPlayerSP) {
    			final ItemStack stack2 = ((EntityPlayerSP) player).inventory.getStackInSlot(Client.INVENTORY_UTIL.itemSlot);
    			if (stack2 != null) {
    				stack = stack2;
    			}
    		}
