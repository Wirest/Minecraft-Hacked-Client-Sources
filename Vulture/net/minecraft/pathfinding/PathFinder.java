...
public class PathFinder
{
    ...
    public PathEntity createEntityPathTo(IBlockAccess blockaccess, Entity entityIn, BlockPos fromPos, BlockPos targetPos, float dist)
    {
        return this.createEntityPathTo(blockaccess, entityIn, fromPos.getX() + .5, fromPos.getY(), fromPos.getZ() + .5, targetPos.getX() + 0.5F, targetPos.getY() + 0.5F, targetPos.getZ() + 0.5F, dist);
    }
    ...
    private PathEntity createEntityPathTo(IBlockAccess blockaccess, Entity entityIn, double x, double y, double z, double x2, double y2, double z2, float distance)
    {
        this.path.clearPath();
        this.nodeProcessor.initProcessor(blockaccess, entityIn);
        PathPoint pathpoint = this.nodeProcessor.getPathPointToCoords(entityIn, x, y, z);
