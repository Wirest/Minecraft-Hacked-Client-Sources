...
public abstract class EntityPlayer extends EntityLivingBase
{
    ...
    public float getToolDigEfficiency(Block p_180471_1_)
    {
        ...
        AutoTool autoTool = (AutoTool) Client.INSTANCE.getModuleManager().getModuleByClass(AutoTool.class);
        
        if (autoTool.isEnabled() && autoTool.getValueByName("AutoToolSilent").getBooleanValue() && Minecraft.getMinecraft().objectMouseOver.typeOfHit == MovingObjectType.BLOCK) {
        	if (Client.INVENTORY_UTIL.itemSlot >= 0 || Client.INVENTORY_UTIL.itemSlot < 9) {
        		BlockPos hoveredBlock = Minecraft.getMinecraft().objectMouseOver.getBlockPos();
        		
        		if (!(hoveredBlock.getBlock() instanceof BlockAir || hoveredBlock == null || hoveredBlock.getBlock() == null)) {
        			ItemStack stack = inventory.getStackInSlot(Client.INVENTORY_UTIL.itemSlot);
        			if (stack != null) {
        				f = inventory.getStackInSlot(Client.INVENTORY_UTIL.itemSlot).getStrVsBlock(hoveredBlock.getBlock());
        			}
        		}
        	}
        }
