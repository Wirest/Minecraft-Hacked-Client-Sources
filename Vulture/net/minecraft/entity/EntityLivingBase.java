...
import de.iotacb.client.events.world.EntityDeathEvent;
...
public abstract class EntityLivingBase extends Entity
{
    ...
    public void handleStatusUpdate(byte id)
    {
        if (id == 2)
        {
        	EventManager.call(new EntityDamageEvent(this));
            this.limbSwingAmount = 1.5F;
            this.hurtResistantTime = this.maxHurtResistantTime;
            this.hurtTime = this.maxHurtTime = 10;
            this.attackedAtYaw = 0.0F;
    ...
    protected void jump()
    {
        ...
        if (this.isSprinting())
        {
        	Sprint sprint = (Sprint) Client.INSTANCE.getModuleManager().getModuleByClass(Sprint.class);
        	
            float f = this.rotationYaw * 0.017453292F;
        	if (sprint.isEnabled() && sprint.getValueByName("SprintAll directions").getBooleanValue()) {
        		f = (float) Client.MOVEMENT_UTIL.getYaw(true);
        	}
    ...
    public boolean canBlockBeSeen(BlockPos pos) {
    	return this.worldObj.rayTraceBlocks(new Vec3(this.posX, this.posY + this.getEyeHeight(), this.posZ), pos.getVec3()) == null;
    }
