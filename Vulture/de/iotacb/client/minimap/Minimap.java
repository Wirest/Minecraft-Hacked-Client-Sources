...
public class Minimap
{
    ...
    public void updateMapFrame(final int bufferSize, final float partial) {
        ...
        final boolean motionBlur = Minecraft.getMinecraft().getDebugFPS() >= 35;
    ...
    public void renderEntityListToFBO(final EntityPlayer p, final ArrayList<Entity> list, final float angle, final double playerX, final double playerZ, final float partial) {
        for (int i = 0; i < list.size(); ++i) {
            final Entity e = list.get(i);
            if (p != e) {
    ...
    public static void renderPlayerHeadToFBO(final EntityPlayer mainPlayer, final AbstractClientPlayer e, final float angle, final double playerX, final double playerZ, final float partial) {
        ...
        final boolean flag1 = e != null && e.isWearing(EnumPlayerModelParts.CAPE) && (e.getGameProfile().getName().equals("Dinnerbone") || e.getGameProfile().getName().equals("Grumm"));
        final double l2 = 8 + (flag1 ? 8 : 0);
        final double i3 = 8 * (flag1 ? -1 : 1);
        Minecraft.getMinecraft().getTextureManager().bindTexture(e.getLocationSkin());
        Gui.drawScaledCustomSizeModalRect(0, 0, 8.0, l2 + i3, 8, -i3, 8, 8, 64.0, 64.0);
        GL11.glPopMatrix();
    }
    ...
    public static void renderEntityDotToFBO(final EntityPlayer p, final Entity e, final float angle, final double playerX, final double playerZ, final float partial) {
        try {
            ...
            GL11.glColor4f(f, f2, f3, 1f);
            GL11.glScalef(2, 2, 1.0f);
            Client.RENDER2D.rect(-2.5, -2.5, 5, 5, Client.INSTANCE.getClientColor());
            GL11.glPopMatrix();
        }
        catch (Exception ex) {
            ex.printStackTrace();
    ...
    static void drawMyTexturedModalRect(final float x, final float y, final int textureX, final int textureY, final float width, final float height, final float factor) {
        final float f2;
        final float f = f2 = 1.0f / factor;
        Tessellator tessellator = Tessellator.getInstance();
        WorldRenderer worldrenderer = tessellator.getWorldRenderer();
        worldrenderer.begin(7, DefaultVertexFormats.POSITION_TEX);
        worldrenderer.pos((double)(x + 0.0f), (double)(y + height), 0.0).tex((double)((textureX + 0) * f), (double)((textureY + height) * f2)).endVertex();
        worldrenderer.pos((double)(x + width), (double)(y + height), 0.0).tex((double)((textureX + width) * f), (double)((textureY + height) * f2)).endVertex();
        worldrenderer.pos((double)(x + width), (double)(y + 0.0f), 0.0).tex((double)((textureX + width) * f), (double)((textureY + 0) * f2)).endVertex();
        worldrenderer.pos((double)(x + 0.0f), (double)(y + 0.0f), 0.0).tex((double)((textureX + 0) * f), (double)((textureY + 0) * f2)).endVertex();
        tessellator.draw();
    }
