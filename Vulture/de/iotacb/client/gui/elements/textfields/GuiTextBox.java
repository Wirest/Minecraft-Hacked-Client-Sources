...
public class GuiTextBox extends GuiTextField {
	...
	public void drawTextBox(int mouseX, int mouseY) {
		Client.RENDER2D.rect(xPosition,  yPosition, width, height, isFocused() ? isHovered(mouseX, mouseY) ? new Color(255, 255, 255, 100) : new Color(255, 255, 255, 50) : isHovered(mouseX, mouseY) ? new Color(40, 40, 40) : new Color(30, 30, 30));
		...
		if (this.cursorCounter / 6 % 2 == 0 && isFocused()) {
			if (cursorPosition == getTextUntrimmed().length()) {
				Client.INSTANCE.getFontManager().getDefaultFont().drawStringWithShadow("_", xPosition + 4 - offsetX + Client.INSTANCE.getFontManager().getDefaultFont().getWidth((hide ? getTextUntrimmed().replaceAll(".", "*") : getText())), yPosition + height / 2 - Client.INSTANCE.getFontManager().getDefaultFont().getHeight("_") / 2, Color.white);
			}
		}
