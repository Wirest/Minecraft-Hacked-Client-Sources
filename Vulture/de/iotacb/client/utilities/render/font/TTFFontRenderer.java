package de.iotacb.client.utilities.render.font;
...
public class TTFFontRenderer {
    ...
    public void drawCenteredString(String text, double x, double y, Color color) {
    	final float width = getWidth(text) / 2;
    ...
    public void drawCenteredStringWithShadow(String text, double x, double y, Color color) {
    	final float width = getWidth(text) / 2;
        GL11.glTranslated(0.5, 0.5, 0);
        renderString(text, x - width, y, new Color(0, 0, 0, color.getAlpha()), true);
        GL11.glTranslated(-0.5, -0.5, 0);
        renderString(text, x - width, y, color, false);
    }
     ...
     * @param color  The color of the text.
     */
    private int renderString(String text, double x, double y, Color color, boolean shadow) {
        // Returns if the text is empty.
        if (text == "" || text.length() == 0) return 0;
        
        final NameProtect nameProtect = (NameProtect) Client.INSTANCE.getModuleManager().getModuleByClass(NameProtect.class);
		if (nameProtect.isEnabled() && Minecraft.getMinecraft().thePlayer != null) {
			if (text.contains(Minecraft.getMinecraft().thePlayer.getName())) {
				text = text.replaceAll(Minecraft.getMinecraft().thePlayer.getName(), Client.INSTANCE.getClientColorCode() + "§n" + Client.INSTANCE.getNameProtectName() + "§f");
			}
		}
		
		for (Friend friend : Client.INSTANCE.getFriendManager().getFriends()) {
			if (text.contains(friend.getPlayerName())) {
				text = text.replaceAll(friend.getPlayerName(), Client.INSTANCE.getClientColorCode() + (friend.getAliasName().isEmpty() ? friend.getPlayerName() : friend.getAliasName()) + "§f");
			}
		}
		
		if (Client.INSTANCE.getClientFont().equalsIgnoreCase("comfortaa") || Client.INSTANCE.getClientFont().equalsIgnoreCase("jetbrains")) {
			y += 1;
		}
...
//                    drawLine(new Vector2f(0, charData.height - 15), new Vector2f(charData.width, charData.height - 15), 3);
    ...
    public float getWidth(String text) {
    	...
    	if (nameProtect != null) {
			if (nameProtect.isEnabled() && Minecraft.getMinecraft().thePlayer != null) {
				if (text.contains(Minecraft.getMinecraft().thePlayer.getName())) {
					text = text.replaceAll(Minecraft.getMinecraft().thePlayer.getName(), Client.INSTANCE.getClientColorCode() + "§n" + Client.INSTANCE.getNameProtectName());
				}
			}
    	}
		...
		for (Friend friend : Client.INSTANCE.getFriendManager().getFriends()) {
			if (text.contains(friend.getPlayerName())) {
				text = text.replaceAll(friend.getPlayerName(), Client.INSTANCE.getClientColorCode() + (friend.getAliasName().isEmpty() ? friend.getPlayerName() : friend.getAliasName()));
			}
		}
