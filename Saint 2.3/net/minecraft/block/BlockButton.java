...
public abstract class BlockButton extends Block {
   ...
   public int isProvidingStrongPower(IBlockAccess worldIn, BlockPos pos, IBlockState state, EnumFacing side) {
      return !(Boolean)state.getValue(POWERED_PROP) ? 0 : (state.getValue(FACING_PROP) == side ? 15 : 0);
   }
   ...
   public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
      if (!worldIn.isRemote && (Boolean)state.getValue(POWERED_PROP)) {
         if (this.wooden) {
   ...
   public void onEntityCollidedWithBlock(World worldIn, BlockPos pos, IBlockState state, Entity entityIn) {
      if (!worldIn.isRemote && this.wooden && !(Boolean)state.getValue(POWERED_PROP)) {
         this.func_180680_f(worldIn, pos, state);
      }
   ...
   public int getMetaFromState(IBlockState state) {
      int var2;
      switch((EnumFacing)state.getValue(FACING_PROP)) {
      case EAST:
         var2 = 1;
         break;
      case WEST:
